...
JoinHint::JoinHint(std::string owner) : owner_(std::move(owner)) {}

void JoinHint::insert(JoinHintEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void JoinHint::remove_before(std::uint64_t tick) {
...
JoinHintDecision JoinHint::evaluate(std::string_view key, double threshold) const {
    JoinHintDecision decision;
    double total = 0.0;
...
std::vector<JoinHintEntry> JoinHint::drain(double minimum_score) const {
    std::vector<JoinHintEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<JoinHintEntry> JoinHint::find(std::string_view key) const {
...
JoinHintDecision merge_join_hint_decisions(const std::vector<JoinHintDecision>& decisions) {
    JoinHintDecision merged;
    if (decisions.empty()) {
        merged.reason = "no join hints";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "join ordering" : "pending evidence";
    return merged;
}

std::string render_join_hint_decision(const JoinHintDecision& decision) {
    std::ostringstream out;
    out << "JoinHint(accepted=" << (decision.accepted ? "true" : "false")
