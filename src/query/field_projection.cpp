...
FieldProjection::FieldProjection(std::string owner) : owner_(std::move(owner)) {}

void FieldProjection::insert(FieldProjectionEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void FieldProjection::remove_before(std::uint64_t tick) {
...
FieldProjectionDecision FieldProjection::evaluate(std::string_view key, double threshold) const {
    FieldProjectionDecision decision;
    double total = 0.0;
...
std::vector<FieldProjectionEntry> FieldProjection::drain(double minimum_score) const {
    std::vector<FieldProjectionEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<FieldProjectionEntry> FieldProjection::find(std::string_view key) const {
...
FieldProjectionDecision merge_field_projection_decisions(const std::vector<FieldProjectionDecision>& decisions) {
    FieldProjectionDecision merged;
    if (decisions.empty()) {
        merged.reason = "no projected fields";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "projection summaries" : "pending evidence";
    return merged;
}

std::string render_field_projection_decision(const FieldProjectionDecision& decision) {
    std::ostringstream out;
    out << "FieldProjection(accepted=" << (decision.accepted ? "true" : "false")
