...
PredicatePlan::PredicatePlan(std::string owner) : owner_(std::move(owner)) {}

void PredicatePlan::insert(PredicatePlanEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void PredicatePlan::remove_before(std::uint64_t tick) {
...
PredicatePlanDecision PredicatePlan::evaluate(std::string_view key, double threshold) const {
    PredicatePlanDecision decision;
    double total = 0.0;
...
std::vector<PredicatePlanEntry> PredicatePlan::drain(double minimum_score) const {
    std::vector<PredicatePlanEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<PredicatePlanEntry> PredicatePlan::find(std::string_view key) const {
...
PredicatePlanDecision merge_predicate_plan_decisions(const std::vector<PredicatePlanDecision>& decisions) {
    PredicatePlanDecision merged;
    if (decisions.empty()) {
        merged.reason = "no query predicates";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "predicate planning" : "pending evidence";
    return merged;
}

std::string render_predicate_plan_decision(const PredicatePlanDecision& decision) {
    std::ostringstream out;
    out << "PredicatePlan(accepted=" << (decision.accepted ? "true" : "false")
