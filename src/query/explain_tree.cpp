...
ExplainTree::ExplainTree(std::string owner) : owner_(std::move(owner)) {}

void ExplainTree::insert(ExplainTreeEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void ExplainTree::remove_before(std::uint64_t tick) {
...
ExplainTreeDecision ExplainTree::evaluate(std::string_view key, double threshold) const {
    ExplainTreeDecision decision;
    double total = 0.0;
...
std::vector<ExplainTreeEntry> ExplainTree::drain(double minimum_score) const {
    std::vector<ExplainTreeEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<ExplainTreeEntry> ExplainTree::find(std::string_view key) const {
...
ExplainTreeDecision merge_explain_tree_decisions(const std::vector<ExplainTreeDecision>& decisions) {
    ExplainTreeDecision merged;
    if (decisions.empty()) {
        merged.reason = "no query plans";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "explain output" : "pending evidence";
    return merged;
}

std::string render_explain_tree_decision(const ExplainTreeDecision& decision) {
    std::ostringstream out;
    out << "ExplainTree(accepted=" << (decision.accepted ? "true" : "false")
