...
MaterializedView::MaterializedView(std::string owner) : owner_(std::move(owner)) {}

void MaterializedView::insert(MaterializedViewEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void MaterializedView::remove_before(std::uint64_t tick) {
...
MaterializedViewDecision MaterializedView::evaluate(std::string_view key, double threshold) const {
    MaterializedViewDecision decision;
    double total = 0.0;
...
std::vector<MaterializedViewEntry> MaterializedView::drain(double minimum_score) const {
    std::vector<MaterializedViewEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<MaterializedViewEntry> MaterializedView::find(std::string_view key) const {
...
MaterializedViewDecision merge_materialized_view_decisions(const std::vector<MaterializedViewDecision>& decisions) {
    MaterializedViewDecision merged;
    if (decisions.empty()) {
        merged.reason = "no view rows";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "view freshness" : "pending evidence";
    return merged;
}

std::string render_materialized_view_decision(const MaterializedViewDecision& decision) {
    std::ostringstream out;
    out << "MaterializedView(accepted=" << (decision.accepted ? "true" : "false")
