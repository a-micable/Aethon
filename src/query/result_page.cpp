...
ResultPage::ResultPage(std::string owner) : owner_(std::move(owner)) {}

void ResultPage::insert(ResultPageEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void ResultPage::remove_before(std::uint64_t tick) {
...
ResultPageDecision ResultPage::evaluate(std::string_view key, double threshold) const {
    ResultPageDecision decision;
    double total = 0.0;
...
std::vector<ResultPageEntry> ResultPage::drain(double minimum_score) const {
    std::vector<ResultPageEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<ResultPageEntry> ResultPage::find(std::string_view key) const {
...
ResultPageDecision merge_result_page_decisions(const std::vector<ResultPageDecision>& decisions) {
    ResultPageDecision merged;
    if (decisions.empty()) {
        merged.reason = "no query results";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "page formation" : "pending evidence";
    return merged;
}

std::string render_result_page_decision(const ResultPageDecision& decision) {
    std::ostringstream out;
    out << "ResultPage(accepted=" << (decision.accepted ? "true" : "false")
