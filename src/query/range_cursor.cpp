...
RangeCursor::RangeCursor(std::string owner) : owner_(std::move(owner)) {}

void RangeCursor::insert(RangeCursorEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void RangeCursor::remove_before(std::uint64_t tick) {
...
RangeCursorDecision RangeCursor::evaluate(std::string_view key, double threshold) const {
    RangeCursorDecision decision;
    double total = 0.0;
...
std::vector<RangeCursorEntry> RangeCursor::drain(double minimum_score) const {
    std::vector<RangeCursorEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<RangeCursorEntry> RangeCursor::find(std::string_view key) const {
...
RangeCursorDecision merge_range_cursor_decisions(const std::vector<RangeCursorDecision>& decisions) {
    RangeCursorDecision merged;
    if (decisions.empty()) {
        merged.reason = "no range scans";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "cursor movement" : "pending evidence";
    return merged;
}

std::string render_range_cursor_decision(const RangeCursorDecision& decision) {
    std::ostringstream out;
    out << "RangeCursor(accepted=" << (decision.accepted ? "true" : "false")
