...
FilterCache::FilterCache(std::string owner) : owner_(std::move(owner)) {}

void FilterCache::insert(FilterCacheEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void FilterCache::remove_before(std::uint64_t tick) {
...
FilterCacheDecision FilterCache::evaluate(std::string_view key, double threshold) const {
    FilterCacheDecision decision;
    double total = 0.0;
...
std::vector<FilterCacheEntry> FilterCache::drain(double minimum_score) const {
    std::vector<FilterCacheEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<FilterCacheEntry> FilterCache::find(std::string_view key) const {
...
FilterCacheDecision merge_filter_cache_decisions(const std::vector<FilterCacheDecision>& decisions) {
    FilterCacheDecision merged;
    if (decisions.empty()) {
        merged.reason = "no filter matches";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "cache hit summaries" : "pending evidence";
    return merged;
}

std::string render_filter_cache_decision(const FilterCacheDecision& decision) {
    std::ostringstream out;
    out << "FilterCache(accepted=" << (decision.accepted ? "true" : "false")
