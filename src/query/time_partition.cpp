...
TimePartition::TimePartition(std::string owner) : owner_(std::move(owner)) {}

void TimePartition::insert(TimePartitionEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void TimePartition::remove_before(std::uint64_t tick) {
...
TimePartitionDecision TimePartition::evaluate(std::string_view key, double threshold) const {
    TimePartitionDecision decision;
    double total = 0.0;
...
std::vector<TimePartitionEntry> TimePartition::drain(double minimum_score) const {
    std::vector<TimePartitionEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<TimePartitionEntry> TimePartition::find(std::string_view key) const {
...
TimePartitionDecision merge_time_partition_decisions(const std::vector<TimePartitionDecision>& decisions) {
    TimePartitionDecision merged;
    if (decisions.empty()) {
        merged.reason = "no time ranges";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "partition lookup" : "pending evidence";
    return merged;
}

std::string render_time_partition_decision(const TimePartitionDecision& decision) {
    std::ostringstream out;
    out << "TimePartition(accepted=" << (decision.accepted ? "true" : "false")
