...
ArchiveManifest::ArchiveManifest(std::string name) : name_(std::move(name)) {}

void ArchiveManifest::observe(ArchiveManifestSample sample) {
...
void ArchiveManifest::clear_before(std::uint64_t timestamp_ns) {
...
ArchiveManifestSummary ArchiveManifest::summarize() const {
    std::vector<ArchiveManifestSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_manifest(copy);
...
std::optional<ArchiveManifestSample> ArchiveManifest::latest(std::string_view label) const {
...
std::vector<ArchiveManifestSample> ArchiveManifest::select(double threshold) const {
    std::vector<ArchiveManifestSample> out;
...
ArchiveManifestSummary summarize_manifest(const std::vector<ArchiveManifestSample>& samples) {
    ArchiveManifestSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no capture metadata observed";
        return summary;
    }
    ...
    summary.note = "stable manifest rendering";
    return summary;
}

double manifest_stability_index(const ArchiveManifestSummary& summary) {
    if (summary.count == 0) {
        return 0.0;
    }
...
std::string describe_manifest(const ArchiveManifestSummary& summary) {
    std::ostringstream out;
    out << "ArchiveManifest[count=" << summary.count
