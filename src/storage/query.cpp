...
ArchiveQuery::ArchiveQuery(std::string name) : name_(std::move(name)) {}

void ArchiveQuery::observe(ArchiveQuerySample sample) {
...
void ArchiveQuery::clear_before(std::uint64_t timestamp_ns) {
...
ArchiveQuerySummary ArchiveQuery::summarize() const {
    std::vector<ArchiveQuerySample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_query(copy);
...
std::optional<ArchiveQuerySample> ArchiveQuery::latest(std::string_view label) const {
...
std::vector<ArchiveQuerySample> ArchiveQuery::select(double threshold) const {
    std::vector<ArchiveQuerySample> out;
...
ArchiveQuerySummary summarize_query(const std::vector<ArchiveQuerySample>& samples) {
    ArchiveQuerySummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no archive ranges observed";
        return summary;
    }
    ...
    summary.note = "predicate filtering";
    return summary;
}

double query_stability_index(const ArchiveQuerySummary& summary) {
    if (summary.count == 0) {
        return 0.0;
    }
...
std::string describe_query(const ArchiveQuerySummary& summary) {
    std::ostringstream out;
    out << "ArchiveQuery[count=" << summary.count
