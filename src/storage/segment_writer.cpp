#include "aethon/storage/segment_writer.hpp"
...
SegmentWriter::SegmentWriter(std::string name) : name_(std::move(name)) {}

void SegmentWriter::observe(SegmentWriterSample sample) {
...
void SegmentWriter::clear_before(std::uint64_t timestamp_ns) {
...
SegmentWriterSummary SegmentWriter::summarize() const {
    std::vector<SegmentWriterSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_segment_writer(copy);
...
std::optional<SegmentWriterSample> SegmentWriter::latest(std::string_view label) const {
...
std::vector<SegmentWriterSample> SegmentWriter::select(double threshold) const {
    std::vector<SegmentWriterSample> out;
...
SegmentWriterSummary summarize_segment_writer(const std::vector<SegmentWriterSample>& samples) {
    SegmentWriterSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no capture segments observed";
        return summary;
    }
    ...
    summary.note = "rollover decisions";
    return summary;
}
...
std::string describe_segment_writer(const SegmentWriterSummary& summary) {
    ...
    out << "SegmentWriter[count=" << summary.count
        << ", age=" << summary.average
