...
BlockCache::BlockCache(std::string name) : name_(std::move(name)) {}

void BlockCache::observe(BlockCacheSample sample) {
...
void BlockCache::clear_before(std::uint64_t timestamp_ns) {
...
BlockCacheSummary BlockCache::summarize() const {
    std::vector<BlockCacheSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_block_cache(copy);
...
std::optional<BlockCacheSample> BlockCache::latest(std::string_view label) const {
...
std::vector<BlockCacheSample> BlockCache::select(double threshold) const {
    std::vector<BlockCacheSample> out;
...
BlockCacheSummary summarize_block_cache(const std::vector<BlockCacheSample>& samples) {
    BlockCacheSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no archive blocks observed";
        return summary;
    }
...
std::string describe_block_cache(const BlockCacheSummary& summary) {
    ...
    out << "BlockCache[count=" << summary.count
