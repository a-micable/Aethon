...
ArchiveWriter::ArchiveWriter(const std::filesystem::path& path) : out_(path, std::ios::binary) { if (!out_) throw Error(ErrorCode::archive_corrupt, "failed to open archive for writing"); write_all(out_, archive_magic); codec::BinaryWriter header; header.u32(summary_.format_version); header.u64(0); header.u64(0); header.u64(0); write_all(out_, header.buffer()); }
...
ArchiveReader::ArchiveReader(const std::filesystem::path& path) : in_(path, std::ios::binary) { if (!in_) throw Error(ErrorCode::archive_corrupt, "failed to open archive for reading"); auto magic = read_exact(in_, archive_magic.size()); if (!std::equal(magic.begin(), magic.end(), archive_magic.begin())) throw Error(ErrorCode::archive_corrupt, "archive magic mismatch"); codec::BinaryReader r(read_exact(in_, 28)); summary_.format_version = r.u32(); summary_.record_count = r.u64(); summary_.first_time_ns = r.u64(); summary_.last_time_ns = r.u64(); }
