#include "aethon/storage/archive_index.hpp"
...
ArchiveIndex::ArchiveIndex(std::string name) : name_(std::move(name)) {}

void ArchiveIndex::observe(ArchiveIndexSample sample) {
...
void ArchiveIndex::clear_before(std::uint64_t timestamp_ns) {
...
ArchiveIndexSummary ArchiveIndex::summarize() const {
    std::vector<ArchiveIndexSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_archive_index(copy);
...
std::optional<ArchiveIndexSample> ArchiveIndex::latest(std::string_view label) const {
...
std::vector<ArchiveIndexSample> ArchiveIndex::select(double threshold) const {
    std::vector<ArchiveIndexSample> out;
...
ArchiveIndexSummary summarize_archive_index(const std::vector<ArchiveIndexSample>& samples) {
    ...
    summary.note = "timestamp offset lookup";
    return summary;
}
...
std::string describe_archive_index(const ArchiveIndexSummary& summary) {
    ...
    out << "ArchiveIndex[count=" << summary.count
