...
ArchiveCompaction::ArchiveCompaction(std::string name) : name_(std::move(name)) {}

void ArchiveCompaction::observe(ArchiveCompactionSample sample) {
...
void ArchiveCompaction::clear_before(std::uint64_t timestamp_ns) {
...
ArchiveCompactionSummary ArchiveCompaction::summarize() const {
    std::vector<ArchiveCompactionSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_compaction(copy);
...
std::optional<ArchiveCompactionSample> ArchiveCompaction::latest(std::string_view label) const {
...
std::vector<ArchiveCompactionSample> ArchiveCompaction::select(double threshold) const {
    std::vector<ArchiveCompactionSample> out;
...
ArchiveCompactionSummary summarize_compaction(const std::vector<ArchiveCompactionSample>& samples) {
    ArchiveCompactionSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no record groups observed";
        return summary;
    }
    ...
    summary.note = "block merge decisions";
    return summary;
}
...
std::string describe_compaction(const ArchiveCompactionSummary& summary) {
    ...
    out << "ArchiveCompaction[count=" << summary.count
