...
RetentionPolicy::RetentionPolicy(std::string name) : name_(std::move(name)) {}

void RetentionPolicy::observe(RetentionPolicySample sample) {
...
void RetentionPolicy::clear_before(std::uint64_t timestamp_ns) {
...
RetentionPolicySummary RetentionPolicy::summarize() const {
    std::vector<RetentionPolicySample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_retention(copy);
...
std::optional<RetentionPolicySample> RetentionPolicy::latest(std::string_view label) const {
...
std::vector<RetentionPolicySample> RetentionPolicy::select(double threshold) const {
    std::vector<RetentionPolicySample> out;
...
RetentionPolicySummary summarize_retention(const std::vector<RetentionPolicySample>& samples) {
    RetentionPolicySummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no archive summaries observed";
        return summary;
    }
    ...
    summary.note = "retention eligibility";
    return summary;
}
...
std::string describe_retention(const RetentionPolicySummary& summary) {
    ...
    out << "RetentionPolicy[count=" << summary.count
