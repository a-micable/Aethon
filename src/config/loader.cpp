...
ConfigLoader::ConfigLoader(std::string name) : name_(std::move(name)) {}

void ConfigLoader::observe(ConfigLoaderSample sample) {
...
void ConfigLoader::clear_before(std::uint64_t timestamp_ns) {
...
ConfigLoaderSummary ConfigLoader::summarize() const {
    std::vector<ConfigLoaderSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_loader(copy);
...
std::optional<ConfigLoaderSample> ConfigLoader::latest(std::string_view label) const {
...
std::vector<ConfigLoaderSample> ConfigLoader::select(double threshold) const {
    std::vector<ConfigLoaderSample> out;
...
ConfigLoaderSummary summarize_loader(const std::vector<ConfigLoaderSample>& samples) {
    ConfigLoaderSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no configuration files observed";
        return summary;
    }
...
std::string describe_loader(const ConfigLoaderSummary& summary) {
    ...
    out << "ConfigLoader[count=" << summary.count
        << ", age=" << summary.average
