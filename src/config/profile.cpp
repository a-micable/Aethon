...
RuntimeProfile::RuntimeProfile(std::string name) : name_(std::move(name)) {}

void RuntimeProfile::observe(RuntimeProfileSample sample) {
...
void RuntimeProfile::clear_before(std::uint64_t timestamp_ns) {
...
RuntimeProfileSummary RuntimeProfile::summarize() const {
    std::vector<RuntimeProfileSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_profile(copy);
...
std::optional<RuntimeProfileSample> RuntimeProfile::latest(std::string_view label) const {
...
std::vector<RuntimeProfileSample> RuntimeProfile::select(double threshold) const {
    std::vector<RuntimeProfileSample> out;
...
RuntimeProfileSummary summarize_profile(const std::vector<RuntimeProfileSample>& samples) {
    ...
    if (samples.empty()) {
        summary.note = "no deployment profiles observed";
        return summary;
    }
...
std::string describe_profile(const RuntimeProfileSummary& summary) {
    ...
    out << "RuntimeProfile[count=" << summary.count
