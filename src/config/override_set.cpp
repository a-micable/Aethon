...
OverrideSet::OverrideSet(std::string name) : name_(std::move(name)) {}

void OverrideSet::observe(OverrideSetSample sample) {
...
void OverrideSet::clear_before(std::uint64_t timestamp_ns) {
...
OverrideSetSummary OverrideSet::summarize() const {
    std::vector<OverrideSetSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_override_set(copy);
...
std::optional<OverrideSetSample> OverrideSet::latest(std::string_view label) const {
...
std::vector<OverrideSetSample> OverrideSet::select(double threshold) const {
    std::vector<OverrideSetSample> out;
...
OverrideSetSummary summarize_override_set(const std::vector<OverrideSetSample>& samples) {
    OverrideSetSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no operator overrides observed";
        return summary;
    }
    ...
    summary.note = "precedence resolution";
    return summary;
}
...
std::string describe_override_set(const OverrideSetSummary& summary) {
    ...
    out << "OverrideSet[count=" << summary.count
