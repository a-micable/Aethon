#include "aethon/config/secret_reference.hpp"
...
SecretReference::SecretReference(std::string name) : name_(std::move(name)) {}

void SecretReference::observe(SecretReferenceSample sample) {
...
void SecretReference::clear_before(std::uint64_t timestamp_ns) {
...
SecretReferenceSummary SecretReference::summarize() const {
    std::vector<SecretReferenceSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_secret_reference(copy);
...
std::optional<SecretReferenceSample> SecretReference::latest(std::string_view label) const {
...
std::vector<SecretReferenceSample> SecretReference::select(double threshold) const {
    std::vector<SecretReferenceSample> out;
...
SecretReferenceSummary summarize_secret_reference(const std::vector<SecretReferenceSample>& samples) {
    SecretReferenceSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no secret handles observed";
        return summary;
    }
    ...
    summary.note = "redacted references";
    return summary;
}

double secret_reference_stability_index(const SecretReferenceSummary& summary) {
    if (summary.count == 0) {
        return 0.0;
    }
...
std::string describe_secret_reference(const SecretReferenceSummary& summary) {
    std::ostringstream out;
    out << "SecretReference[count=" << summary.count
