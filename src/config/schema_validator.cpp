#include "aethon/config/schema_validator.hpp"
...
SchemaValidator::SchemaValidator(std::string name) : name_(std::move(name)) {}

void SchemaValidator::observe(SchemaValidatorSample sample) {
...
void SchemaValidator::clear_before(std::uint64_t timestamp_ns) {
...
SchemaValidatorSummary SchemaValidator::summarize() const {
    std::vector<SchemaValidatorSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_schema_validator(copy);
...
std::optional<SchemaValidatorSample> SchemaValidator::latest(std::string_view label) const {
...
std::vector<SchemaValidatorSample> SchemaValidator::select(double threshold) const {
    std::vector<SchemaValidatorSample> out;
...
SchemaValidatorSummary summarize_schema_validator(const std::vector<SchemaValidatorSample>& samples) {
    SchemaValidatorSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no configuration keys observed";
        return summary;
    }
    ...
    summary.note = "validation reports";
    return summary;
}
...
std::string describe_schema_validator(const SchemaValidatorSummary& summary) {
    ...
    out << "SchemaValidator[count=" << summary.count
