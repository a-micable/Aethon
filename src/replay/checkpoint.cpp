#include "aethon/replay/checkpoint.hpp"
...
ReplayCheckpoint::ReplayCheckpoint(std::string name) : name_(std::move(name)) {}

void ReplayCheckpoint::observe(ReplayCheckpointSample sample) {
...
void ReplayCheckpoint::clear_before(std::uint64_t timestamp_ns) {
...
ReplayCheckpointSummary ReplayCheckpoint::summarize() const {
    std::vector<ReplayCheckpointSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_checkpoint(copy);
...
std::optional<ReplayCheckpointSample> ReplayCheckpoint::latest(std::string_view label) const {
...
std::vector<ReplayCheckpointSample> ReplayCheckpoint::select(double threshold) const {
    std::vector<ReplayCheckpointSample> out;
...
ReplayCheckpointSummary summarize_checkpoint(const std::vector<ReplayCheckpointSample>& samples) {
    ReplayCheckpointSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no replay offsets observed";
        return summary;
    }
    ...
    summary.note = "resume tokens";
    return summary;
}

double checkpoint_stability_index(const ReplayCheckpointSummary& summary) {
    if (summary.count == 0) {
        return 0.0;
    }
...
std::string describe_checkpoint(const ReplayCheckpointSummary& summary) {
    std::ostringstream out;
    out << "ReplayCheckpoint[count=" << summary.count
