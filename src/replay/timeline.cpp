#include "aethon/replay/timeline.hpp"
...
ReplayTimeline::ReplayTimeline(std::string name) : name_(std::move(name)) {}

void ReplayTimeline::observe(ReplayTimelineSample sample) {
...
void ReplayTimeline::clear_before(std::uint64_t timestamp_ns) {
...
ReplayTimelineSummary ReplayTimeline::summarize() const {
    std::vector<ReplayTimelineSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_timeline(copy);
...
std::optional<ReplayTimelineSample> ReplayTimeline::latest(std::string_view label) const {
...
std::vector<ReplayTimelineSample> ReplayTimeline::select(double threshold) const {
    std::vector<ReplayTimelineSample> out;
...
ReplayTimelineSummary summarize_timeline(const std::vector<ReplayTimelineSample>& samples) {
    ReplayTimelineSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no archive records observed";
        return summary;
    }
    ...
    summary.note = "time ordered playback";
    return summary;
}
...
std::string describe_timeline(const ReplayTimelineSummary& summary) {
    ...
    out << "ReplayTimeline[count=" << summary.count
