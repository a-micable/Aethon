#include "aethon/replay/cursor.hpp"
...
ReplayCursor::ReplayCursor(std::string name) : name_(std::move(name)) {}

void ReplayCursor::observe(ReplayCursorSample sample) {
...
void ReplayCursor::clear_before(std::uint64_t timestamp_ns) {
...
ReplayCursorSummary ReplayCursor::summarize() const {
    std::vector<ReplayCursorSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_cursor(copy);
...
std::optional<ReplayCursorSample> ReplayCursor::latest(std::string_view label) const {
...
std::vector<ReplayCursorSample> ReplayCursor::select(double threshold) const {
    std::vector<ReplayCursorSample> out;
...
ReplayCursorSummary summarize_cursor(const std::vector<ReplayCursorSample>& samples) {
    ReplayCursorSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no archive windows observed";
        return summary;
    }
    ...
    summary.note = "cursor progression";
    return summary;
}
...
std::string describe_cursor(const ReplayCursorSummary& summary) {
    ...
    out << "ReplayCursor[count=" << summary.count
        << ", age=" << summary.average
