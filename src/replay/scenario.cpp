...
ReplayScenario::ReplayScenario(std::string name) : name_(std::move(name)) {}

void ReplayScenario::observe(ReplayScenarioSample sample) {
...
void ReplayScenario::clear_before(std::uint64_t timestamp_ns) {
...
ReplayScenarioSummary ReplayScenario::summarize() const {
    std::vector<ReplayScenarioSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_scenario(copy);
...
std::optional<ReplayScenarioSample> ReplayScenario::latest(std::string_view label) const {
...
std::vector<ReplayScenarioSample> ReplayScenario::select(double threshold) const {
    std::vector<ReplayScenarioSample> out;
...
ReplayScenarioSummary summarize_scenario(const std::vector<ReplayScenarioSample>& samples) {
    ReplayScenarioSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no operator scripts observed";
        return summary;
    }
    ...
    summary.note = "scenario validation";
    return summary;
}
...
std::string describe_scenario(const ReplayScenarioSummary& summary) {
    ...
    out << "ReplayScenario[count=" << summary.count
