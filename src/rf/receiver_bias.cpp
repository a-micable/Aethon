...
ReceiverBias::ReceiverBias(std::string owner) : owner_(std::move(owner)) {}

void ReceiverBias::insert(ReceiverBiasEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void ReceiverBias::remove_before(std::uint64_t tick) {
...
ReceiverBiasDecision ReceiverBias::evaluate(std::string_view key, double threshold) const {
    ReceiverBiasDecision decision;
    double total = 0.0;
...
std::vector<ReceiverBiasEntry> ReceiverBias::drain(double minimum_score) const {
    std::vector<ReceiverBiasEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<ReceiverBiasEntry> ReceiverBias::find(std::string_view key) const {
...
ReceiverBiasDecision merge_receiver_bias_decisions(const std::vector<ReceiverBiasDecision>& decisions) {
    ReceiverBiasDecision merged;
    if (decisions.empty()) {
        merged.reason = "no receiver offsets";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "bias correction" : "pending evidence";
    return merged;
}

std::string render_receiver_bias_decision(const ReceiverBiasDecision& decision) {
    std::ostringstream out;
    out << "ReceiverBias(accepted=" << (decision.accepted ? "true" : "false")
