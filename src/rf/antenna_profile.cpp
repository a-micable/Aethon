#include "aethon/rf/antenna_profile.hpp"
...
AntennaProfile::AntennaProfile(std::string owner) : owner_(std::move(owner)) {}

void AntennaProfile::insert(AntennaProfileEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void AntennaProfile::remove_before(std::uint64_t tick) {
...
AntennaProfileDecision AntennaProfile::evaluate(std::string_view key, double threshold) const {
    AntennaProfileDecision decision;
    double total = 0.0;
...
std::vector<AntennaProfileEntry> AntennaProfile::drain(double minimum_score) const {
    std::vector<AntennaProfileEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<AntennaProfileEntry> AntennaProfile::find(std::string_view key) const {
...
AntennaProfileDecision merge_antenna_profile_decisions(const std::vector<AntennaProfileDecision>& decisions) {
    AntennaProfileDecision merged;
    if (decisions.empty()) {
        merged.reason = "no antenna samples";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "gain summaries" : "pending evidence";
    return merged;
}

std::string render_antenna_profile_decision(const AntennaProfileDecision& decision) {
    std::ostringstream out;
    out << "AntennaProfile(accepted=" << (decision.accepted ? "true" : "false")
