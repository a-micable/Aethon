...
GainSchedule::GainSchedule(std::string owner) : owner_(std::move(owner)) {}

void GainSchedule::insert(GainScheduleEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void GainSchedule::remove_before(std::uint64_t tick) {
...
GainScheduleDecision GainSchedule::evaluate(std::string_view key, double threshold) const {
    GainScheduleDecision decision;
    double total = 0.0;
...
std::vector<GainScheduleEntry> GainSchedule::drain(double minimum_score) const {
    std::vector<GainScheduleEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<GainScheduleEntry> GainSchedule::find(std::string_view key) const {
...
GainScheduleDecision merge_gain_schedule_decisions(const std::vector<GainScheduleDecision>& decisions) {
    GainScheduleDecision merged;
    if (decisions.empty()) {
        merged.reason = "no gain changes";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "schedule validation" : "pending evidence";
    return merged;
}

std::string render_gain_schedule_decision(const GainScheduleDecision& decision) {
    std::ostringstream out;
    out << "GainSchedule(accepted=" << (decision.accepted ? "true" : "false")
