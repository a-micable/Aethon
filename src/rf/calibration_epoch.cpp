...
CalibrationEpoch::CalibrationEpoch(std::string owner) : owner_(std::move(owner)) {}

void CalibrationEpoch::insert(CalibrationEpochEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void CalibrationEpoch::remove_before(std::uint64_t tick) {
...
CalibrationEpochDecision CalibrationEpoch::evaluate(std::string_view key, double threshold) const {
    CalibrationEpochDecision decision;
    double total = 0.0;
...
std::vector<CalibrationEpochEntry> CalibrationEpoch::drain(double minimum_score) const {
    std::vector<CalibrationEpochEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<CalibrationEpochEntry> CalibrationEpoch::find(std::string_view key) const {
...
CalibrationEpochDecision merge_calibration_epoch_decisions(const std::vector<CalibrationEpochDecision>& decisions) {
    CalibrationEpochDecision merged;
    if (decisions.empty()) {
        merged.reason = "no calibration epochs";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "epoch validity" : "pending evidence";
    return merged;
}

std::string render_calibration_epoch_decision(const CalibrationEpochDecision& decision) {
    std::ostringstream out;
    out << "CalibrationEpoch(accepted=" << (decision.accepted ? "true" : "false")
