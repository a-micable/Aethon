...
PhaseModel::PhaseModel(std::string owner) : owner_(std::move(owner)) {}

void PhaseModel::insert(PhaseModelEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void PhaseModel::remove_before(std::uint64_t tick) {
...
PhaseModelDecision PhaseModel::evaluate(std::string_view key, double threshold) const {
    PhaseModelDecision decision;
    double total = 0.0;
...
std::vector<PhaseModelEntry> PhaseModel::drain(double minimum_score) const {
    std::vector<PhaseModelEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<PhaseModelEntry> PhaseModel::find(std::string_view key) const {
...
PhaseModelDecision merge_phase_model_decisions(const std::vector<PhaseModelDecision>& decisions) {
    PhaseModelDecision merged;
    if (decisions.empty()) {
        merged.reason = "no phase observations";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "phase drift" : "pending evidence";
    return merged;
}

std::string render_phase_model_decision(const PhaseModelDecision& decision) {
    std::ostringstream out;
    out << "PhaseModel(accepted=" << (decision.accepted ? "true" : "false")
