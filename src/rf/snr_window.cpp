#include "aethon/rf/snr_window.hpp"
...
SnrWindow::SnrWindow(std::string owner) : owner_(std::move(owner)) {}

void SnrWindow::insert(SnrWindowEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void SnrWindow::remove_before(std::uint64_t tick) {
...
SnrWindowDecision SnrWindow::evaluate(std::string_view key, double threshold) const {
    SnrWindowDecision decision;
    double total = 0.0;
...
std::vector<SnrWindowEntry> SnrWindow::drain(double minimum_score) const {
    std::vector<SnrWindowEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<SnrWindowEntry> SnrWindow::find(std::string_view key) const {
...
SnrWindowDecision merge_snr_window_decisions(const std::vector<SnrWindowDecision>& decisions) {
    SnrWindowDecision merged;
    if (decisions.empty()) {
        merged.reason = "no snr samples";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "noise separation" : "pending evidence";
    return merged;
}

std::string render_snr_window_decision(const SnrWindowDecision& decision) {
    std::ostringstream out;
    out << "SnrWindow(accepted=" << (decision.accepted ? "true" : "false")
