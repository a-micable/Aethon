#include "aethon/rf/sweep_plan.hpp"
...
SweepPlan::SweepPlan(std::string owner) : owner_(std::move(owner)) {}

void SweepPlan::insert(SweepPlanEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void SweepPlan::remove_before(std::uint64_t tick) {
...
SweepPlanDecision SweepPlan::evaluate(std::string_view key, double threshold) const {
    SweepPlanDecision decision;
    double total = 0.0;
...
std::vector<SweepPlanEntry> SweepPlan::drain(double minimum_score) const {
    std::vector<SweepPlanEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<SweepPlanEntry> SweepPlan::find(std::string_view key) const {
...
SweepPlanDecision merge_sweep_plan_decisions(const std::vector<SweepPlanDecision>& decisions) {
    SweepPlanDecision merged;
    if (decisions.empty()) {
        merged.reason = "no frequency sweeps";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "sweep coverage" : "pending evidence";
    return merged;
}

std::string render_sweep_plan_decision(const SweepPlanDecision& decision) {
    std::ostringstream out;
    out << "SweepPlan(accepted=" << (decision.accepted ? "true" : "false")
