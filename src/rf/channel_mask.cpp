#include "aethon/rf/channel_mask.hpp"
...
ChannelMask::ChannelMask(std::string owner) : owner_(std::move(owner)) {}

void ChannelMask::insert(ChannelMaskEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void ChannelMask::remove_before(std::uint64_t tick) {
...
ChannelMaskDecision ChannelMask::evaluate(std::string_view key, double threshold) const {
    ChannelMaskDecision decision;
    double total = 0.0;
...
std::vector<ChannelMaskEntry> ChannelMask::drain(double minimum_score) const {
    std::vector<ChannelMaskEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<ChannelMaskEntry> ChannelMask::find(std::string_view key) const {
...
ChannelMaskDecision merge_channel_mask_decisions(const std::vector<ChannelMaskDecision>& decisions) {
    ChannelMaskDecision merged;
    if (decisions.empty()) {
        merged.reason = "no channel masks";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "allowed channel selection" : "pending evidence";
    return merged;
}

std::string render_channel_mask_decision(const ChannelMaskDecision& decision) {
    std::ostringstream out;
    out << "ChannelMask(accepted=" << (decision.accepted ? "true" : "false")
