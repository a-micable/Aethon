...
RssiTracker::RssiTracker(std::string owner) : owner_(std::move(owner)) {}

void RssiTracker::insert(RssiTrackerEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void RssiTracker::remove_before(std::uint64_t tick) {
...
RssiTrackerDecision RssiTracker::evaluate(std::string_view key, double threshold) const {
    RssiTrackerDecision decision;
    double total = 0.0;
...
std::vector<RssiTrackerEntry> RssiTracker::drain(double minimum_score) const {
    std::vector<RssiTrackerEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<RssiTrackerEntry> RssiTracker::find(std::string_view key) const {
...
RssiTrackerDecision merge_rssi_tracker_decisions(const std::vector<RssiTrackerDecision>& decisions) {
    RssiTrackerDecision merged;
    if (decisions.empty()) {
        merged.reason = "no rssi samples";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "signal strength trends" : "pending evidence";
    return merged;
}

std::string render_rssi_tracker_decision(const RssiTrackerDecision& decision) {
    std::ostringstream out;
    out << "RssiTracker(accepted=" << (decision.accepted ? "true" : "false")
