...
RuleParser::RuleParser(std::string name) : name_(std::move(name)) {}

void RuleParser::observe(RuleParserSample sample) {
...
void RuleParser::clear_before(std::uint64_t timestamp_ns) {
...
RuleParserSummary RuleParser::summarize() const {
    std::vector<RuleParserSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_rule_parser(copy);
...
std::optional<RuleParserSample> RuleParser::latest(std::string_view label) const {
...
std::vector<RuleParserSample> RuleParser::select(double threshold) const {
    std::vector<RuleParserSample> out;
...
RuleParserSummary summarize_rule_parser(const std::vector<RuleParserSample>& samples) {
    RuleParserSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no routing rules observed";
        return summary;
    }
...
std::string describe_rule_parser(const RuleParserSummary& summary) {
    ...
    out << "RuleParser[count=" << summary.count
