...
DeadLetterQueue::DeadLetterQueue(std::string name) : name_(std::move(name)) {}

void DeadLetterQueue::observe(DeadLetterQueueSample sample) {
...
void DeadLetterQueue::clear_before(std::uint64_t timestamp_ns) {
...
DeadLetterQueueSummary DeadLetterQueue::summarize() const {
    std::vector<DeadLetterQueueSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_dead_letter(copy);
...
std::optional<DeadLetterQueueSample> DeadLetterQueue::latest(std::string_view label) const {
...
std::vector<DeadLetterQueueSample> DeadLetterQueue::select(double threshold) const {
    std::vector<DeadLetterQueueSample> out;
...
DeadLetterQueueSummary summarize_dead_letter(const std::vector<DeadLetterQueueSample>& samples) {
    DeadLetterQueueSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no failed routes observed";
        return summary;
    }
...
std::string describe_dead_letter(const DeadLetterQueueSummary& summary) {
    ...
    out << "DeadLetterQueue[count=" << summary.count
