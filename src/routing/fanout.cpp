#include "aethon/routing/fanout.hpp"
...
FanoutPlan::FanoutPlan(std::string name) : name_(std::move(name)) {}

void FanoutPlan::observe(FanoutPlanSample sample) {
...
void FanoutPlan::clear_before(std::uint64_t timestamp_ns) {
...
FanoutPlanSummary FanoutPlan::summarize() const {
    std::vector<FanoutPlanSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_fanout(copy);
...
std::optional<FanoutPlanSample> FanoutPlan::latest(std::string_view label) const {
...
std::vector<FanoutPlanSample> FanoutPlan::select(double threshold) const {
    std::vector<FanoutPlanSample> out;
...
FanoutPlanSummary summarize_fanout(const std::vector<FanoutPlanSample>& samples) {
    FanoutPlanSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no route decisions observed";
        return summary;
    }
    ...
    summary.note = "multi-sink fanout";
    return summary;
}

double fanout_stability_index(const FanoutPlanSummary& summary) {
    if (summary.count == 0) {
        return 0.0;
    }
...
std::string describe_fanout(const FanoutPlanSummary& summary) {
    std::ostringstream out;
    out << "FanoutPlan[count=" << summary.count
