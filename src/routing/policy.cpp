#include "aethon/routing/policy.hpp"
...
RoutingPolicy::RoutingPolicy(std::string name) : name_(std::move(name)) {}

void RoutingPolicy::observe(RoutingPolicySample sample) {
...
void RoutingPolicy::clear_before(std::uint64_t timestamp_ns) {
...
RoutingPolicySummary RoutingPolicy::summarize() const {
    std::vector<RoutingPolicySample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_policy(copy);
...
std::optional<RoutingPolicySample> RoutingPolicy::latest(std::string_view label) const {
...
std::vector<RoutingPolicySample> RoutingPolicy::select(double threshold) const {
    std::vector<RoutingPolicySample> out;
...
RoutingPolicySummary summarize_policy(const std::vector<RoutingPolicySample>& samples) {
    RoutingPolicySummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no metadata observed";
        return summary;
    }
    ...
    summary.note = "policy decisions";
    return summary;
}

double policy_stability_index(const RoutingPolicySummary& summary) {
    if (summary.count == 0) {
        return 0.0;
    }
...
std::string describe_policy(const RoutingPolicySummary& summary) {
    std::ostringstream out;
    out << "RoutingPolicy[count=" << summary.count
