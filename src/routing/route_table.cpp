...
RouteTable::RouteTable(std::string name) : name_(std::move(name)) {}

void RouteTable::observe(RouteTableSample sample) {
...
void RouteTable::clear_before(std::uint64_t timestamp_ns) {
...
RouteTableSummary RouteTable::summarize() const {
    std::vector<RouteTableSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_route_table(copy);
...
std::optional<RouteTableSample> RouteTable::latest(std::string_view label) const {
...
std::vector<RouteTableSample> RouteTable::select(double threshold) const {
    std::vector<RouteTableSample> out;
...
RouteTableSummary summarize_route_table(const std::vector<RouteTableSample>& samples) {
    RouteTableSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no route entries observed";
        return summary;
    }
    ...
    summary.note = "collector selection";
    return summary;
}
...
std::string describe_route_table(const RouteTableSummary& summary) {
    ...
    out << "RouteTable[count=" << summary.count
