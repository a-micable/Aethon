#include "aethon/routing/priority_queue.hpp"
...
PriorityQueue::PriorityQueue(std::string name) : name_(std::move(name)) {}

void PriorityQueue::observe(PriorityQueueSample sample) {
...
void PriorityQueue::clear_before(std::uint64_t timestamp_ns) {
...
PriorityQueueSummary PriorityQueue::summarize() const {
    std::vector<PriorityQueueSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_priority_queue(copy);
...
std::optional<PriorityQueueSample> PriorityQueue::latest(std::string_view label) const {
...
std::vector<PriorityQueueSample> PriorityQueue::select(double threshold) const {
    std::vector<PriorityQueueSample> out;
...
PriorityQueueSummary summarize_priority_queue(const std::vector<PriorityQueueSample>& samples) {
    PriorityQueueSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no pending packets observed";
        return summary;
    }
    ...
    summary.note = "priority scheduling";
    return summary;
}
...
std::string describe_priority_queue(const PriorityQueueSummary& summary) {
    ...
    out << "PriorityQueue[count=" << summary.count
