...
RegionMap::RegionMap(std::string name) : name_(std::move(name)) {}

void RegionMap::observe(RegionMapSample sample) {
...
void RegionMap::clear_before(std::uint64_t timestamp_ns) {
...
RegionMapSummary RegionMap::summarize() const {
    std::vector<RegionMapSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_region_map(copy);
...
std::optional<RegionMapSample> RegionMap::latest(std::string_view label) const {
...
std::vector<RegionMapSample> RegionMap::select(double threshold) const {
    std::vector<RegionMapSample> out;
...
RegionMapSummary summarize_region_map(const std::vector<RegionMapSample>& samples) {
    RegionMapSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no collectors observed";
        return summary;
    }
    ...
    summary.note = "regional ownership";
    return summary;
}
...
std::string describe_region_map(const RegionMapSummary& summary) {
    ...
    out << "RegionMap[count=" << summary.count
