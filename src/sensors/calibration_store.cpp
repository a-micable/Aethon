...
CalibrationStore::CalibrationStore(std::string name) : name_(std::move(name)) {}

void CalibrationStore::observe(CalibrationStoreSample sample) {
...
void CalibrationStore::clear_before(std::uint64_t timestamp_ns) {
...
CalibrationStoreSummary CalibrationStore::summarize() const {
    std::vector<CalibrationStoreSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_calibration_store(copy);
...
std::optional<CalibrationStoreSample> CalibrationStore::latest(std::string_view label) const {
...
std::vector<CalibrationStoreSample> CalibrationStore::select(double threshold) const {
    std::vector<CalibrationStoreSample> out;
...
CalibrationStoreSummary summarize_calibration_store(const std::vector<CalibrationStoreSample>& samples) {
    ...
    if (samples.empty()) {
        summary.note = "no calibration records observed";
        return summary;
    }
    ...
    summary.note = "latest record lookup";
    return summary;
}
...
std::string describe_calibration_store(const CalibrationStoreSummary& summary) {
    ...
    out << "CalibrationStore[count=" << summary.count
