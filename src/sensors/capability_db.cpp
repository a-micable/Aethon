...
CapabilityDb::CapabilityDb(std::string name) : name_(std::move(name)) {}

void CapabilityDb::observe(CapabilityDbSample sample) {
...
void CapabilityDb::clear_before(std::uint64_t timestamp_ns) {
...
CapabilityDbSummary CapabilityDb::summarize() const {
    std::vector<CapabilityDbSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_capability_db(copy);
...
std::optional<CapabilityDbSample> CapabilityDb::latest(std::string_view label) const {
...
std::vector<CapabilityDbSample> CapabilityDb::select(double threshold) const {
    std::vector<CapabilityDbSample> out;
...
CapabilityDbSummary summarize_capability_db(const std::vector<CapabilityDbSample>& samples) {
    CapabilityDbSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no device features observed";
        return summary;
    }
    ...
    summary.note = "capability matching";
    return summary;
}
...
std::string describe_capability_db(const CapabilityDbSummary& summary) {
    ...
    out << "CapabilityDb[count=" << summary.count
