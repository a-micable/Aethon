#include "aethon/sensors/device_registry.hpp"
...
DeviceRegistry::DeviceRegistry(std::string name) : name_(std::move(name)) {}

void DeviceRegistry::observe(DeviceRegistrySample sample) {
...
void DeviceRegistry::clear_before(std::uint64_t timestamp_ns) {
...
DeviceRegistrySummary DeviceRegistry::summarize() const {
    std::vector<DeviceRegistrySample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_device_registry(copy);
...
std::optional<DeviceRegistrySample> DeviceRegistry::latest(std::string_view label) const {
...
std::vector<DeviceRegistrySample> DeviceRegistry::select(double threshold) const {
    std::vector<DeviceRegistrySample> out;
...
DeviceRegistrySummary summarize_device_registry(const std::vector<DeviceRegistrySample>& samples) {
    DeviceRegistrySummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no sensor devices observed";
        return summary;
    }
    ...
    summary.note = "registration lookup";
    return summary;
}
...
std::string describe_device_registry(const DeviceRegistrySummary& summary) {
    ...
    out << "DeviceRegistry[count=" << summary.count
