#include "aethon/sensors/firmware_catalog.hpp"
...
FirmwareCatalog::FirmwareCatalog(std::string name) : name_(std::move(name)) {}

void FirmwareCatalog::observe(FirmwareCatalogSample sample) {
...
void FirmwareCatalog::clear_before(std::uint64_t timestamp_ns) {
...
FirmwareCatalogSummary FirmwareCatalog::summarize() const {
    std::vector<FirmwareCatalogSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_firmware_catalog(copy);
...
std::optional<FirmwareCatalogSample> FirmwareCatalog::latest(std::string_view label) const {
...
std::vector<FirmwareCatalogSample> FirmwareCatalog::select(double threshold) const {
    std::vector<FirmwareCatalogSample> out;
...
FirmwareCatalogSummary summarize_firmware_catalog(const std::vector<FirmwareCatalogSample>& samples) {
    FirmwareCatalogSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no firmware builds observed";
        return summary;
    }
    ...
    summary.note = "version labels";
    return summary;
}

double firmware_catalog_stability_index(const FirmwareCatalogSummary& summary) {
    if (summary.count == 0) {
        return 0.0;
    }
...
std::string describe_firmware_catalog(const FirmwareCatalogSummary& summary) {
    std::ostringstream out;
    out << "FirmwareCatalog[count=" << summary.count
