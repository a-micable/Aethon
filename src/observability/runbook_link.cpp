...
RunbookLink::RunbookLink(std::string owner) : owner_(std::move(owner)) {}

void RunbookLink::insert(RunbookLinkEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void RunbookLink::remove_before(std::uint64_t tick) {
...
RunbookLinkDecision RunbookLink::evaluate(std::string_view key, double threshold) const {
    RunbookLinkDecision decision;
    double total = 0.0;
...
std::vector<RunbookLinkEntry> RunbookLink::drain(double minimum_score) const {
    std::vector<RunbookLinkEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<RunbookLinkEntry> RunbookLink::find(std::string_view key) const {
...
RunbookLinkDecision merge_runbook_link_decisions(const std::vector<RunbookLinkDecision>& decisions) {
    RunbookLinkDecision merged;
    if (decisions.empty()) {
        merged.reason = "no runbook links";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "link lookup" : "pending evidence";
    return merged;
}

std::string render_runbook_link_decision(const RunbookLinkDecision& decision) {
    std::ostringstream out;
    out << "RunbookLink(accepted=" << (decision.accepted ? "true" : "false")
