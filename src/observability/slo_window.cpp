#include "aethon/observability/slo_window.hpp"
...
SloWindow::SloWindow(std::string owner) : owner_(std::move(owner)) {}

void SloWindow::insert(SloWindowEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void SloWindow::remove_before(std::uint64_t tick) {
...
SloWindowDecision SloWindow::evaluate(std::string_view key, double threshold) const {
    SloWindowDecision decision;
    double total = 0.0;
...
std::vector<SloWindowEntry> SloWindow::drain(double minimum_score) const {
    std::vector<SloWindowEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<SloWindowEntry> SloWindow::find(std::string_view key) const {
...
SloWindowDecision merge_slo_window_decisions(const std::vector<SloWindowDecision>& decisions) {
    SloWindowDecision merged;
    if (decisions.empty()) {
        merged.reason = "no slo samples";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "objective tracking" : "pending evidence";
    return merged;
}

std::string render_slo_window_decision(const SloWindowDecision& decision) {
    std::ostringstream out;
    out << "SloWindow(accepted=" << (decision.accepted ? "true" : "false")
