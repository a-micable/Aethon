...
SpanSampler::SpanSampler(std::string owner) : owner_(std::move(owner)) {}

void SpanSampler::insert(SpanSamplerEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void SpanSampler::remove_before(std::uint64_t tick) {
...
SpanSamplerDecision SpanSampler::evaluate(std::string_view key, double threshold) const {
    SpanSamplerDecision decision;
    double total = 0.0;
...
std::vector<SpanSamplerEntry> SpanSampler::drain(double minimum_score) const {
    std::vector<SpanSamplerEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<SpanSamplerEntry> SpanSampler::find(std::string_view key) const {
...
SpanSamplerDecision merge_span_sampler_decisions(const std::vector<SpanSamplerDecision>& decisions) {
    SpanSamplerDecision merged;
    if (decisions.empty()) {
        merged.reason = "no trace spans";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "sampling decisions" : "pending evidence";
    return merged;
}

std::string render_span_sampler_decision(const SpanSamplerDecision& decision) {
    std::ostringstream out;
    out << "SpanSampler(accepted=" << (decision.accepted ? "true" : "false")
