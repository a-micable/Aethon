...
ScrapeTarget::ScrapeTarget(std::string owner) : owner_(std::move(owner)) {}

void ScrapeTarget::insert(ScrapeTargetEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void ScrapeTarget::remove_before(std::uint64_t tick) {
...
ScrapeTargetDecision ScrapeTarget::evaluate(std::string_view key, double threshold) const {
    ScrapeTargetDecision decision;
    double total = 0.0;
...
std::vector<ScrapeTargetEntry> ScrapeTarget::drain(double minimum_score) const {
    std::vector<ScrapeTargetEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<ScrapeTargetEntry> ScrapeTarget::find(std::string_view key) const {
...
ScrapeTargetDecision merge_scrape_target_decisions(const std::vector<ScrapeTargetDecision>& decisions) {
    ScrapeTargetDecision merged;
    if (decisions.empty()) {
        merged.reason = "no scrape targets";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "target health" : "pending evidence";
    return merged;
}

std::string render_scrape_target_decision(const ScrapeTargetDecision& decision) {
    std::ostringstream out;
    out << "ScrapeTarget(accepted=" << (decision.accepted ? "true" : "false")
