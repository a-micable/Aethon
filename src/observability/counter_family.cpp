...
CounterFamily::CounterFamily(std::string owner) : owner_(std::move(owner)) {}

void CounterFamily::insert(CounterFamilyEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void CounterFamily::remove_before(std::uint64_t tick) {
...
CounterFamilyDecision CounterFamily::evaluate(std::string_view key, double threshold) const {
    CounterFamilyDecision decision;
    double total = 0.0;
...
std::vector<CounterFamilyEntry> CounterFamily::drain(double minimum_score) const {
    std::vector<CounterFamilyEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<CounterFamilyEntry> CounterFamily::find(std::string_view key) const {
...
CounterFamilyDecision merge_counter_family_decisions(const std::vector<CounterFamilyDecision>& decisions) {
    CounterFamilyDecision merged;
    if (decisions.empty()) {
        merged.reason = "no metric counters";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "counter export" : "pending evidence";
    return merged;
}

std::string render_counter_family_decision(const CounterFamilyDecision& decision) {
    std::ostringstream out;
    out << "CounterFamily(accepted=" << (decision.accepted ? "true" : "false")
