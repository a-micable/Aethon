...
BurnRate::BurnRate(std::string owner) : owner_(std::move(owner)) {}

void BurnRate::insert(BurnRateEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void BurnRate::remove_before(std::uint64_t tick) {
...
BurnRateDecision BurnRate::evaluate(std::string_view key, double threshold) const {
    BurnRateDecision decision;
    double total = 0.0;
...
std::vector<BurnRateEntry> BurnRate::drain(double minimum_score) const {
    std::vector<BurnRateEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<BurnRateEntry> BurnRate::find(std::string_view key) const {
...
BurnRateDecision merge_burn_rate_decisions(const std::vector<BurnRateDecision>& decisions) {
    BurnRateDecision merged;
    if (decisions.empty()) {
        merged.reason = "no error budget burn";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "burn rate scoring" : "pending evidence";
    return merged;
}

std::string render_burn_rate_decision(const BurnRateDecision& decision) {
    std::ostringstream out;
    out << "BurnRate(accepted=" << (decision.accepted ? "true" : "false")
