...
TelemetryBatch::TelemetryBatch(std::string owner) : owner_(std::move(owner)) {}

void TelemetryBatch::insert(TelemetryBatchEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void TelemetryBatch::remove_before(std::uint64_t tick) {
...
TelemetryBatchDecision TelemetryBatch::evaluate(std::string_view key, double threshold) const {
    TelemetryBatchDecision decision;
    double total = 0.0;
...
std::vector<TelemetryBatchEntry> TelemetryBatch::drain(double minimum_score) const {
    std::vector<TelemetryBatchEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<TelemetryBatchEntry> TelemetryBatch::find(std::string_view key) const {
...
TelemetryBatchDecision merge_telemetry_batch_decisions(const std::vector<TelemetryBatchDecision>& decisions) {
    TelemetryBatchDecision merged;
    if (decisions.empty()) {
        merged.reason = "no telemetry batches";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "batch summaries" : "pending evidence";
    return merged;
}

std::string render_telemetry_batch_decision(const TelemetryBatchDecision& decision) {
    std::ostringstream out;
    out << "TelemetryBatch(accepted=" << (decision.accepted ? "true" : "false")
