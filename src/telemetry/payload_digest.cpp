...
PayloadDigest::PayloadDigest(std::string owner) : owner_(std::move(owner)) {}

void PayloadDigest::insert(PayloadDigestEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void PayloadDigest::remove_before(std::uint64_t tick) {
...
PayloadDigestDecision PayloadDigest::evaluate(std::string_view key, double threshold) const {
    PayloadDigestDecision decision;
    double total = 0.0;
...
std::vector<PayloadDigestEntry> PayloadDigest::drain(double minimum_score) const {
    std::vector<PayloadDigestEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<PayloadDigestEntry> PayloadDigest::find(std::string_view key) const {
...
PayloadDigestDecision merge_payload_digest_decisions(const std::vector<PayloadDigestDecision>& decisions) {
    PayloadDigestDecision merged;
    if (decisions.empty()) {
        merged.reason = "no payload digests";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "digest summaries" : "pending evidence";
    return merged;
}

std::string render_payload_digest_decision(const PayloadDigestDecision& decision) {
    std::ostringstream out;
    out << "PayloadDigest(accepted=" << (decision.accepted ? "true" : "false")
