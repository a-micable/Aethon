#include "aethon/telemetry/device_note.hpp"
...
DeviceNote::DeviceNote(std::string owner) : owner_(std::move(owner)) {}

void DeviceNote::insert(DeviceNoteEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void DeviceNote::remove_before(std::uint64_t tick) {
...
DeviceNoteDecision DeviceNote::evaluate(std::string_view key, double threshold) const {
    DeviceNoteDecision decision;
    double total = 0.0;
...
std::vector<DeviceNoteEntry> DeviceNote::drain(double minimum_score) const {
    std::vector<DeviceNoteEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<DeviceNoteEntry> DeviceNote::find(std::string_view key) const {
...
DeviceNoteDecision merge_device_note_decisions(const std::vector<DeviceNoteDecision>& decisions) {
    DeviceNoteDecision merged;
    if (decisions.empty()) {
        merged.reason = "no operator notes";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "note search" : "pending evidence";
    return merged;
}

std::string render_device_note_decision(const DeviceNoteDecision& decision) {
    std::ostringstream out;
    out << "DeviceNote(accepted=" << (decision.accepted ? "true" : "false")
