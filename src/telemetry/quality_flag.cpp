...
QualityFlag::QualityFlag(std::string owner) : owner_(std::move(owner)) {}

void QualityFlag::insert(QualityFlagEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void QualityFlag::remove_before(std::uint64_t tick) {
...
QualityFlagDecision QualityFlag::evaluate(std::string_view key, double threshold) const {
    QualityFlagDecision decision;
    double total = 0.0;
...
std::vector<QualityFlagEntry> QualityFlag::drain(double minimum_score) const {
    std::vector<QualityFlagEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<QualityFlagEntry> QualityFlag::find(std::string_view key) const {
...
QualityFlagDecision merge_quality_flag_decisions(const std::vector<QualityFlagDecision>& decisions) {
    QualityFlagDecision merged;
    if (decisions.empty()) {
        merged.reason = "no quality flags";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "flag rollups" : "pending evidence";
    return merged;
}

std::string render_quality_flag_decision(const QualityFlagDecision& decision) {
    std::ostringstream out;
    out << "QualityFlag(accepted=" << (decision.accepted ? "true" : "false")
