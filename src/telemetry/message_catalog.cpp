...
MessageCatalog::MessageCatalog(std::string owner) : owner_(std::move(owner)) {}

void MessageCatalog::insert(MessageCatalogEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void MessageCatalog::remove_before(std::uint64_t tick) {
...
MessageCatalogDecision MessageCatalog::evaluate(std::string_view key, double threshold) const {
    MessageCatalogDecision decision;
    double total = 0.0;
...
std::vector<MessageCatalogEntry> MessageCatalog::drain(double minimum_score) const {
    std::vector<MessageCatalogEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<MessageCatalogEntry> MessageCatalog::find(std::string_view key) const {
...
MessageCatalogDecision merge_message_catalog_decisions(const std::vector<MessageCatalogDecision>& decisions) {
    MessageCatalogDecision merged;
    if (decisions.empty()) {
        merged.reason = "no message ids";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "catalog lookup" : "pending evidence";
    return merged;
}

std::string render_message_catalog_decision(const MessageCatalogDecision& decision) {
    std::ostringstream out;
    out << "MessageCatalog(accepted=" << (decision.accepted ? "true" : "false")
