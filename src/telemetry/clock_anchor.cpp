...
ClockAnchor::ClockAnchor(std::string owner) : owner_(std::move(owner)) {}

void ClockAnchor::insert(ClockAnchorEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void ClockAnchor::remove_before(std::uint64_t tick) {
...
ClockAnchorDecision ClockAnchor::evaluate(std::string_view key, double threshold) const {
    ClockAnchorDecision decision;
    double total = 0.0;
...
std::vector<ClockAnchorEntry> ClockAnchor::drain(double minimum_score) const {
    std::vector<ClockAnchorEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<ClockAnchorEntry> ClockAnchor::find(std::string_view key) const {
...
ClockAnchorDecision merge_clock_anchor_decisions(const std::vector<ClockAnchorDecision>& decisions) {
    ClockAnchorDecision merged;
    if (decisions.empty()) {
        merged.reason = "no clock anchors";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "time alignment" : "pending evidence";
    return merged;
}

std::string render_clock_anchor_decision(const ClockAnchorDecision& decision) {
    std::ostringstream out;
    out << "ClockAnchor(accepted=" << (decision.accepted ? "true" : "false")
