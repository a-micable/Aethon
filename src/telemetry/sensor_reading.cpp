...
SensorReading::SensorReading(std::string owner) : owner_(std::move(owner)) {}

void SensorReading::insert(SensorReadingEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void SensorReading::remove_before(std::uint64_t tick) {
...
SensorReadingDecision SensorReading::evaluate(std::string_view key, double threshold) const {
    SensorReadingDecision decision;
    double total = 0.0;
...
std::vector<SensorReadingEntry> SensorReading::drain(double minimum_score) const {
    std::vector<SensorReadingEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<SensorReadingEntry> SensorReading::find(std::string_view key) const {
...
SensorReadingDecision merge_sensor_reading_decisions(const std::vector<SensorReadingDecision>& decisions) {
    SensorReadingDecision merged;
    if (decisions.empty()) {
        merged.reason = "no sensor values";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "reading normalization" : "pending evidence";
    return merged;
}

std::string render_sensor_reading_decision(const SensorReadingDecision& decision) {
    std::ostringstream out;
    out << "SensorReading(accepted=" << (decision.accepted ? "true" : "false")
