...
WatermarkTracker::WatermarkTracker(std::string name) : name_(std::move(name)) {}

void WatermarkTracker::observe(WatermarkTrackerSample sample) {
...
void WatermarkTracker::clear_before(std::uint64_t timestamp_ns) {
...
WatermarkTrackerSummary WatermarkTracker::summarize() const {
    std::vector<WatermarkTrackerSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_watermark_tracker(copy);
...
std::optional<WatermarkTrackerSample> WatermarkTracker::latest(std::string_view label) const {
...
std::vector<WatermarkTrackerSample> WatermarkTracker::select(double threshold) const {
    std::vector<WatermarkTrackerSample> out;
...
WatermarkTrackerSummary summarize_watermark_tracker(const std::vector<WatermarkTrackerSample>& samples) {
    WatermarkTrackerSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no event time observed";
        return summary;
    }
    ...
    summary.note = "late packet tracking";
    return summary;
}

double watermark_tracker_stability_index(const WatermarkTrackerSummary& summary) {
    if (summary.count == 0) {
        return 0.0;
    }
...
std::string describe_watermark_tracker(const WatermarkTrackerSummary& summary) {
    std::ostringstream out;
    out << "WatermarkTracker[count=" << summary.count
