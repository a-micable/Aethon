#include "aethon/stream/frame_synchronizer.hpp"
...
FrameSynchronizer::FrameSynchronizer(std::string name) : name_(std::move(name)) {}

void FrameSynchronizer::observe(FrameSynchronizerSample sample) {
...
void FrameSynchronizer::clear_before(std::uint64_t timestamp_ns) {
...
FrameSynchronizerSummary FrameSynchronizer::summarize() const {
    std::vector<FrameSynchronizerSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_frame_synchronizer(copy);
...
std::optional<FrameSynchronizerSample> FrameSynchronizer::latest(std::string_view label) const {
...
std::vector<FrameSynchronizerSample> FrameSynchronizer::select(double threshold) const {
    std::vector<FrameSynchronizerSample> out;
...
FrameSynchronizerSummary summarize_frame_synchronizer(const std::vector<FrameSynchronizerSample>& samples) {
    FrameSynchronizerSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no raw link bytes observed";
        return summary;
    }
    ...
    summary.note = "magic scanning and losses";
    return summary;
}
...
std::string describe_frame_synchronizer(const FrameSynchronizerSummary& summary) {
    ...
    out << "FrameSynchronizer[count=" << summary.count
