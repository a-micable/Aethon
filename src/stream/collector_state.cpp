#include "aethon/stream/collector_state.hpp"
...
CollectorState::CollectorState(std::string name) : name_(std::move(name)) {}

void CollectorState::observe(CollectorStateSample sample) {
...
void CollectorState::clear_before(std::uint64_t timestamp_ns) {
...
CollectorStateSummary CollectorState::summarize() const {
    std::vector<CollectorStateSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_collector_state(copy);
...
std::optional<CollectorStateSample> CollectorState::latest(std::string_view label) const {
...
std::vector<CollectorStateSample> CollectorState::select(double threshold) const {
    std::vector<CollectorStateSample> out;
...
CollectorStateSummary summarize_collector_state(const std::vector<CollectorStateSample>& samples) {
    CollectorStateSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no device sessions observed";
        return summary;
    }
    ...
    summary.note = "stream lifecycle tracking";
    return summary;
}
...
std::string describe_collector_state(const CollectorStateSummary& summary) {
    ...
    out << "CollectorState[count=" << summary.count
        << ", age=" << summary.average
