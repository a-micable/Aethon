...
LinkQuality::LinkQuality(std::string name) : name_(std::move(name)) {}

void LinkQuality::observe(LinkQualitySample sample) {
...
void LinkQuality::clear_before(std::uint64_t timestamp_ns) {
...
LinkQualitySummary LinkQuality::summarize() const {
    std::vector<LinkQualitySample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_link_quality(copy);
...
std::optional<LinkQualitySample> LinkQuality::latest(std::string_view label) const {
...
std::vector<LinkQualitySample> LinkQuality::select(double threshold) const {
    std::vector<LinkQualitySample> out;
...
LinkQualitySummary summarize_link_quality(const std::vector<LinkQualitySample>& samples) {
    LinkQualitySummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no receiver counters observed";
        return summary;
    }
    ...
    summary.note = "quality trends";
    return summary;
}

double link_quality_stability_index(const LinkQualitySummary& summary) {
    if (summary.count == 0) {
        return 0.0;
    }
...
std::string describe_link_quality(const LinkQualitySummary& summary) {
    std::ostringstream out;
    out << "LinkQuality[count=" << summary.count
