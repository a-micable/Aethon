...
JitterBuffer::JitterBuffer(std::string name) : name_(std::move(name)) {}

void JitterBuffer::observe(JitterBufferSample sample) {
...
void JitterBuffer::clear_before(std::uint64_t timestamp_ns) {
...
JitterBufferSummary JitterBuffer::summarize() const {
    std::vector<JitterBufferSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_jitter_buffer(copy);
...
std::optional<JitterBufferSample> JitterBuffer::latest(std::string_view label) const {
...
std::vector<JitterBufferSample> JitterBuffer::select(double threshold) const {
    std::vector<JitterBufferSample> out;
...
JitterBufferSummary summarize_jitter_buffer(const std::vector<JitterBufferSample>& samples) {
    JitterBufferSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no timestamped packets observed";
        return summary;
    }
    ...
    summary.note = "bounded delay smoothing";
    return summary;
}
...
std::string describe_jitter_buffer(const JitterBufferSummary& summary) {
    ...
    out << "JitterBuffer[count=" << summary.count
