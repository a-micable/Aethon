...
IngestWindow::IngestWindow(std::string name) : name_(std::move(name)) {}

void IngestWindow::observe(IngestWindowSample sample) {
...
void IngestWindow::clear_before(std::uint64_t timestamp_ns) {
...
IngestWindowSummary IngestWindow::summarize() const {
    std::vector<IngestWindowSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_ingest_window(copy);
...
std::optional<IngestWindowSample> IngestWindow::latest(std::string_view label) const {
...
std::vector<IngestWindowSample> IngestWindow::select(double threshold) const {
    std::vector<IngestWindowSample> out;
...
IngestWindowSummary summarize_ingest_window(const std::vector<IngestWindowSample>& samples) {
    IngestWindowSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no ingest batches observed";
        return summary;
    }
    ...
    summary.note = "bounded packet windows";
    return summary;
}
...
std::string describe_ingest_window(const IngestWindowSummary& summary) {
    ...
    out << "IngestWindow[count=" << summary.count
