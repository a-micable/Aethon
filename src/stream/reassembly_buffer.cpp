...
ReassemblyBuffer::ReassemblyBuffer(std::string name) : name_(std::move(name)) {}

void ReassemblyBuffer::observe(ReassemblyBufferSample sample) {
...
void ReassemblyBuffer::clear_before(std::uint64_t timestamp_ns) {
...
ReassemblyBufferSummary ReassemblyBuffer::summarize() const {
    std::vector<ReassemblyBufferSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_reassembly_buffer(copy);
...
std::optional<ReassemblyBufferSample> ReassemblyBuffer::latest(std::string_view label) const {
...
std::vector<ReassemblyBufferSample> ReassemblyBuffer::select(double threshold) const {
    std::vector<ReassemblyBufferSample> out;
...
ReassemblyBufferSummary summarize_reassembly_buffer(const std::vector<ReassemblyBufferSample>& samples) {
    ReassemblyBufferSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no fragments observed";
        return summary;
    }
    ...
    summary.note = "ordered message recovery";
    return summary;
}
...
std::string describe_reassembly_buffer(const ReassemblyBufferSummary& summary) {
    ...
    out << "ReassemblyBuffer[count=" << summary.count
