...
BackpressureController::BackpressureController(std::string name) : name_(std::move(name)) {}

void BackpressureController::observe(BackpressureControllerSample sample) {
...
void BackpressureController::clear_before(std::uint64_t timestamp_ns) {
...
BackpressureControllerSummary BackpressureController::summarize() const {
    std::vector<BackpressureControllerSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_backpressure(copy);
...
std::optional<BackpressureControllerSample> BackpressureController::latest(std::string_view label) const {
...
std::vector<BackpressureControllerSample> BackpressureController::select(double threshold) const {
    std::vector<BackpressureControllerSample> out;
...
BackpressureControllerSummary summarize_backpressure(const std::vector<BackpressureControllerSample>& samples) {
    BackpressureControllerSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no queue levels observed";
        return summary;
    }
    ...
    summary.note = "drop and throttle decisions";
    return summary;
}
...
std::string describe_backpressure(const BackpressureControllerSummary& summary) {
    ...
    out << "BackpressureController[count=" << summary.count
