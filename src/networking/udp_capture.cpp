#include "aethon/networking/udp_capture.hpp"
...
UdpCapture::UdpCapture(std::string name) : name_(std::move(name)) {}

void UdpCapture::observe(UdpCaptureSample sample) {
...
void UdpCapture::clear_before(std::uint64_t timestamp_ns) {
...
UdpCaptureSummary UdpCapture::summarize() const {
    std::vector<UdpCaptureSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_udp_capture(copy);
...
std::optional<UdpCaptureSample> UdpCapture::latest(std::string_view label) const {
...
std::vector<UdpCaptureSample> UdpCapture::select(double threshold) const {
    std::vector<UdpCaptureSample> out;
...
UdpCaptureSummary summarize_udp_capture(const std::vector<UdpCaptureSample>& samples) {
    UdpCaptureSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no datagrams observed";
        return summary;
    }
    ...
    summary.note = "capture accounting";
    return summary;
}
...
std::string describe_udp_capture(const UdpCaptureSummary& summary) {
    ...
    out << "UdpCapture[count=" << summary.count
