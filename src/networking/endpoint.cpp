#include "aethon/networking/endpoint.hpp"
...
Endpoint::Endpoint(std::string name) : name_(std::move(name)) {}

void Endpoint::observe(EndpointSample sample) {
...
void Endpoint::clear_before(std::uint64_t timestamp_ns) {
...
EndpointSummary Endpoint::summarize() const {
    std::vector<EndpointSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_endpoint(copy);
...
std::optional<EndpointSample> Endpoint::latest(std::string_view label) const {
...
std::vector<EndpointSample> Endpoint::select(double threshold) const {
    std::vector<EndpointSample> out;
...
EndpointSummary summarize_endpoint(const std::vector<EndpointSample>& samples) {
    ...
    if (samples.empty()) {
        summary.note = "no collector endpoints observed";
        return summary;
    }
    ...
    summary.note = "address normalization";
    return summary;
}
...
std::string describe_endpoint(const EndpointSummary& summary) {
    ...
    out << "Endpoint[count=" << summary.count
