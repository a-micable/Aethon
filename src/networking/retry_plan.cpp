#include "aethon/networking/retry_plan.hpp"
...
RetryPlan::RetryPlan(std::string name) : name_(std::move(name)) {}

void RetryPlan::observe(RetryPlanSample sample) {
...
void RetryPlan::clear_before(std::uint64_t timestamp_ns) {
...
RetryPlanSummary RetryPlan::summarize() const {
    std::vector<RetryPlanSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_retry_plan(copy);
...
std::optional<RetryPlanSample> RetryPlan::latest(std::string_view label) const {
...
std::vector<RetryPlanSample> RetryPlan::select(double threshold) const {
    std::vector<RetryPlanSample> out;
...
RetryPlanSummary summarize_retry_plan(const std::vector<RetryPlanSample>& samples) {
    RetryPlanSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no transport errors observed";
        return summary;
    }
    ...
    summary.note = "retry scheduling";
    return summary;
}

double retry_plan_stability_index(const RetryPlanSummary& summary) {
    if (summary.count == 0) {
        return 0.0;
    }
...
std::string describe_retry_plan(const RetryPlanSummary& summary) {
    std::ostringstream out;
    out << "RetryPlan[count=" << summary.count
