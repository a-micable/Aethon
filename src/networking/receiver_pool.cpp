...
ReceiverPool::ReceiverPool(std::string name) : name_(std::move(name)) {}

void ReceiverPool::observe(ReceiverPoolSample sample) {
...
void ReceiverPool::clear_before(std::uint64_t timestamp_ns) {
...
ReceiverPoolSummary ReceiverPool::summarize() const {
    std::vector<ReceiverPoolSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_receiver_pool(copy);
...
std::optional<ReceiverPoolSample> ReceiverPool::latest(std::string_view label) const {
...
std::vector<ReceiverPoolSample> ReceiverPool::select(double threshold) const {
    std::vector<ReceiverPoolSample> out;
...
ReceiverPoolSummary summarize_receiver_pool(const std::vector<ReceiverPoolSample>& samples) {
    ReceiverPoolSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no receiver workers observed";
        return summary;
    }
...
std::string describe_receiver_pool(const ReceiverPoolSummary& summary) {
    ...
    out << "ReceiverPool[count=" << summary.count
        << ", age=" << summary.average
