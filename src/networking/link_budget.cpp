#include "aethon/networking/link_budget.hpp"
...
LinkBudget::LinkBudget(std::string name) : name_(std::move(name)) {}

void LinkBudget::observe(LinkBudgetSample sample) {
...
void LinkBudget::clear_before(std::uint64_t timestamp_ns) {
...
LinkBudgetSummary LinkBudget::summarize() const {
    std::vector<LinkBudgetSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_link_budget(copy);
...
std::optional<LinkBudgetSample> LinkBudget::latest(std::string_view label) const {
...
std::vector<LinkBudgetSample> LinkBudget::select(double threshold) const {
    std::vector<LinkBudgetSample> out;
...
LinkBudgetSummary summarize_link_budget(const std::vector<LinkBudgetSample>& samples) {
    LinkBudgetSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no rf link metrics observed";
        return summary;
    }
    ...
    summary.note = "budget estimation";
    return summary;
}
...
std::string describe_link_budget(const LinkBudgetSummary& summary) {
    ...
    out << "LinkBudget[count=" << summary.count
