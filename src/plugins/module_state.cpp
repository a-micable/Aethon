#include "aethon/plugins/module_state.hpp"
...
ModuleState::ModuleState(std::string name) : name_(std::move(name)) {}

void ModuleState::observe(ModuleStateSample sample) {
...
void ModuleState::clear_before(std::uint64_t timestamp_ns) {
...
ModuleStateSummary ModuleState::summarize() const {
    std::vector<ModuleStateSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_module_state(copy);
...
std::optional<ModuleStateSample> ModuleState::latest(std::string_view label) const {
...
std::vector<ModuleStateSample> ModuleState::select(double threshold) const {
    std::vector<ModuleStateSample> out;
...
ModuleStateSummary summarize_module_state(const std::vector<ModuleStateSample>& samples) {
    ...
    if (samples.empty()) {
        summary.note = "no loaded plugins observed";
        return summary;
    }
...
std::string describe_module_state(const ModuleStateSummary& summary) {
    ...
    out << "ModuleState[count=" << summary.count
