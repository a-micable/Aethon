...
HookChain::HookChain(std::string name) : name_(std::move(name)) {}

void HookChain::observe(HookChainSample sample) {
...
void HookChain::clear_before(std::uint64_t timestamp_ns) {
...
HookChainSummary HookChain::summarize() const {
    std::vector<HookChainSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_hook_chain(copy);
...
std::optional<HookChainSample> HookChain::latest(std::string_view label) const {
...
std::vector<HookChainSample> HookChain::select(double threshold) const {
    std::vector<HookChainSample> out;
...
HookChainSummary summarize_hook_chain(const std::vector<HookChainSample>& samples) {
    HookChainSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no packet hooks observed";
        return summary;
    }
    ...
    summary.note = "ordered hook execution";
    return summary;
}
...
std::string describe_hook_chain(const HookChainSummary& summary) {
    ...
    out << "HookChain[count=" << summary.count
