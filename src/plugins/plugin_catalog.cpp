#include "aethon/plugins/plugin_catalog.hpp"
...
PluginCatalog::PluginCatalog(std::string name) : name_(std::move(name)) {}

void PluginCatalog::observe(PluginCatalogSample sample) {
...
void PluginCatalog::clear_before(std::uint64_t timestamp_ns) {
...
PluginCatalogSummary PluginCatalog::summarize() const {
    std::vector<PluginCatalogSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_plugin_catalog(copy);
...
std::optional<PluginCatalogSample> PluginCatalog::latest(std::string_view label) const {
...
std::vector<PluginCatalogSample> PluginCatalog::select(double threshold) const {
    std::vector<PluginCatalogSample> out;
...
PluginCatalogSummary summarize_plugin_catalog(const std::vector<PluginCatalogSample>& samples) {
    PluginCatalogSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no plugin descriptors observed";
        return summary;
    }
    ...
    summary.note = "discovery filtering";
    return summary;
}
...
std::string describe_plugin_catalog(const PluginCatalogSummary& summary) {
    ...
    out << "PluginCatalog[count=" << summary.count
