...
SandboxPolicy::SandboxPolicy(std::string name) : name_(std::move(name)) {}

void SandboxPolicy::observe(SandboxPolicySample sample) {
...
void SandboxPolicy::clear_before(std::uint64_t timestamp_ns) {
...
SandboxPolicySummary SandboxPolicy::summarize() const {
    std::vector<SandboxPolicySample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_sandbox_policy(copy);
...
std::optional<SandboxPolicySample> SandboxPolicy::latest(std::string_view label) const {
...
std::vector<SandboxPolicySample> SandboxPolicy::select(double threshold) const {
    std::vector<SandboxPolicySample> out;
...
SandboxPolicySummary summarize_sandbox_policy(const std::vector<SandboxPolicySample>& samples) {
    ...
    if (samples.empty()) {
        summary.note = "no plugin metadata observed";
        return summary;
    }
    ...
    summary.note = "capability limits";
    return summary;
}
...
std::string describe_sandbox_policy(const SandboxPolicySummary& summary) {
    ...
    out << "SandboxPolicy[count=" << summary.count
