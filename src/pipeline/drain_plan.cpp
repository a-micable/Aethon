#include "aethon/pipeline/drain_plan.hpp"
...
DrainPlan::DrainPlan(std::string owner) : owner_(std::move(owner)) {}

void DrainPlan::insert(DrainPlanEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void DrainPlan::remove_before(std::uint64_t tick) {
...
DrainPlanDecision DrainPlan::evaluate(std::string_view key, double threshold) const {
    DrainPlanDecision decision;
    double total = 0.0;
...
std::vector<DrainPlanEntry> DrainPlan::drain(double minimum_score) const {
    std::vector<DrainPlanEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<DrainPlanEntry> DrainPlan::find(std::string_view key) const {
...
DrainPlanDecision merge_drain_plan_decisions(const std::vector<DrainPlanDecision>& decisions) {
    DrainPlanDecision merged;
    if (decisions.empty()) {
        merged.reason = "no shutdown drains";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "orderly drain planning" : "pending evidence";
    return merged;
}

std::string render_drain_plan_decision(const DrainPlanDecision& decision) {
    std::ostringstream out;
    out << "DrainPlan(accepted=" << (decision.accepted ? "true" : "false")
