...
BatchCursor::BatchCursor(std::string owner) : owner_(std::move(owner)) {}

void BatchCursor::insert(BatchCursorEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void BatchCursor::remove_before(std::uint64_t tick) {
...
BatchCursorDecision BatchCursor::evaluate(std::string_view key, double threshold) const {
    BatchCursorDecision decision;
    double total = 0.0;
...
std::vector<BatchCursorEntry> BatchCursor::drain(double minimum_score) const {
    std::vector<BatchCursorEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<BatchCursorEntry> BatchCursor::find(std::string_view key) const {
...
BatchCursorDecision merge_batch_cursor_decisions(const std::vector<BatchCursorDecision>& decisions) {
    BatchCursorDecision merged;
    if (decisions.empty()) {
        merged.reason = "no capture batches";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "cursor lag and completion" : "pending evidence";
    return merged;
}

std::string render_batch_cursor_decision(const BatchCursorDecision& decision) {
    std::ostringstream out;
    out << "BatchCursor(accepted=" << (decision.accepted ? "true" : "false")
