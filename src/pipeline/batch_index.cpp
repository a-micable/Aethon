#include "aethon/pipeline/batch_index.hpp"
...
BatchIndex::BatchIndex(std::string owner) : owner_(std::move(owner)) {}

void BatchIndex::insert(BatchIndexEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void BatchIndex::remove_before(std::uint64_t tick) {
...
BatchIndexDecision BatchIndex::evaluate(std::string_view key, double threshold) const {
    BatchIndexDecision decision;
    double total = 0.0;
...
std::vector<BatchIndexEntry> BatchIndex::drain(double minimum_score) const {
    std::vector<BatchIndexEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<BatchIndexEntry> BatchIndex::find(std::string_view key) const {
...
BatchIndexDecision merge_batch_index_decisions(const std::vector<BatchIndexDecision>& decisions) {
    BatchIndexDecision merged;
    if (decisions.empty()) {
        merged.reason = "no batch ids";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "lookup stability" : "pending evidence";
    return merged;
}

std::string render_batch_index_decision(const BatchIndexDecision& decision) {
    std::ostringstream out;
    out << "BatchIndex(accepted=" << (decision.accepted ? "true" : "false")
