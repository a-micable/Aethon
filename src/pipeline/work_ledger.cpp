...
WorkLedger::WorkLedger(std::string owner) : owner_(std::move(owner)) {}

void WorkLedger::insert(WorkLedgerEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void WorkLedger::remove_before(std::uint64_t tick) {
...
WorkLedgerDecision WorkLedger::evaluate(std::string_view key, double threshold) const {
    WorkLedgerDecision decision;
    double total = 0.0;
...
std::vector<WorkLedgerEntry> WorkLedger::drain(double minimum_score) const {
    std::vector<WorkLedgerEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<WorkLedgerEntry> WorkLedger::find(std::string_view key) const {
...
WorkLedgerDecision merge_work_ledger_decisions(const std::vector<WorkLedgerDecision>& decisions) {
    WorkLedgerDecision merged;
    if (decisions.empty()) {
        merged.reason = "no worker tasks";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "task accountability" : "pending evidence";
    return merged;
}

std::string render_work_ledger_decision(const WorkLedgerDecision& decision) {
    std::ostringstream out;
    out << "WorkLedger(accepted=" << (decision.accepted ? "true" : "false")
