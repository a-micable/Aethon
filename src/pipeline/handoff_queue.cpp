#include "aethon/pipeline/handoff_queue.hpp"
...
HandoffQueue::HandoffQueue(std::string owner) : owner_(std::move(owner)) {}

void HandoffQueue::insert(HandoffQueueEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void HandoffQueue::remove_before(std::uint64_t tick) {
...
HandoffQueueDecision HandoffQueue::evaluate(std::string_view key, double threshold) const {
    HandoffQueueDecision decision;
    double total = 0.0;
...
std::vector<HandoffQueueEntry> HandoffQueue::drain(double minimum_score) const {
    std::vector<HandoffQueueEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<HandoffQueueEntry> HandoffQueue::find(std::string_view key) const {
...
HandoffQueueDecision merge_handoff_queue_decisions(const std::vector<HandoffQueueDecision>& decisions) {
    HandoffQueueDecision merged;
    if (decisions.empty()) {
        merged.reason = "no handoff events";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "queue pressure" : "pending evidence";
    return merged;
}

std::string render_handoff_queue_decision(const HandoffQueueDecision& decision) {
    std::ostringstream out;
    out << "HandoffQueue(accepted=" << (decision.accepted ? "true" : "false")
