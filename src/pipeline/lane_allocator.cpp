...
LaneAllocator::LaneAllocator(std::string owner) : owner_(std::move(owner)) {}

void LaneAllocator::insert(LaneAllocatorEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void LaneAllocator::remove_before(std::uint64_t tick) {
...
LaneAllocatorDecision LaneAllocator::evaluate(std::string_view key, double threshold) const {
    LaneAllocatorDecision decision;
    double total = 0.0;
...
std::vector<LaneAllocatorEntry> LaneAllocator::drain(double minimum_score) const {
    std::vector<LaneAllocatorEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<LaneAllocatorEntry> LaneAllocator::find(std::string_view key) const {
...
LaneAllocatorDecision merge_lane_allocator_decisions(const std::vector<LaneAllocatorDecision>& decisions) {
    LaneAllocatorDecision merged;
    if (decisions.empty()) {
        merged.reason = "no processing lanes";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "lane balancing" : "pending evidence";
    return merged;
}

std::string render_lane_allocator_decision(const LaneAllocatorDecision& decision) {
    std::ostringstream out;
    out << "LaneAllocator(accepted=" << (decision.accepted ? "true" : "false")
