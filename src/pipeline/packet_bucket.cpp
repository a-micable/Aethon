...
PacketBucket::PacketBucket(std::string owner) : owner_(std::move(owner)) {}

void PacketBucket::insert(PacketBucketEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void PacketBucket::remove_before(std::uint64_t tick) {
...
PacketBucketDecision PacketBucket::evaluate(std::string_view key, double threshold) const {
    PacketBucketDecision decision;
    double total = 0.0;
...
std::vector<PacketBucketEntry> PacketBucket::drain(double minimum_score) const {
    std::vector<PacketBucketEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<PacketBucketEntry> PacketBucket::find(std::string_view key) const {
...
PacketBucketDecision merge_packet_bucket_decisions(const std::vector<PacketBucketDecision>& decisions) {
    PacketBucketDecision merged;
    if (decisions.empty()) {
        merged.reason = "no packet groups";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "bucket rollover" : "pending evidence";
    return merged;
}

std::string render_packet_bucket_decision(const PacketBucketDecision& decision) {
    std::ostringstream out;
    out << "PacketBucket(accepted=" << (decision.accepted ? "true" : "false")
