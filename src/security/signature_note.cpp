...
SignatureNote::SignatureNote(std::string owner) : owner_(std::move(owner)) {}

void SignatureNote::insert(SignatureNoteEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void SignatureNote::remove_before(std::uint64_t tick) {
...
SignatureNoteDecision SignatureNote::evaluate(std::string_view key, double threshold) const {
    SignatureNoteDecision decision;
    double total = 0.0;
...
std::vector<SignatureNoteEntry> SignatureNote::drain(double minimum_score) const {
    std::vector<SignatureNoteEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<SignatureNoteEntry> SignatureNote::find(std::string_view key) const {
...
SignatureNoteDecision merge_signature_note_decisions(const std::vector<SignatureNoteDecision>& decisions) {
    SignatureNoteDecision merged;
    if (decisions.empty()) {
        merged.reason = "no signatures";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "signature labels" : "pending evidence";
    return merged;
}

std::string render_signature_note_decision(const SignatureNoteDecision& decision) {
    std::ostringstream out;
    out << "SignatureNote(accepted=" << (decision.accepted ? "true" : "false")
