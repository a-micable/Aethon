...
KeyEpoch::KeyEpoch(std::string owner) : owner_(std::move(owner)) {}

void KeyEpoch::insert(KeyEpochEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void KeyEpoch::remove_before(std::uint64_t tick) {
...
KeyEpochDecision KeyEpoch::evaluate(std::string_view key, double threshold) const {
    KeyEpochDecision decision;
    double total = 0.0;
...
std::vector<KeyEpochEntry> KeyEpoch::drain(double minimum_score) const {
    std::vector<KeyEpochEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<KeyEpochEntry> KeyEpoch::find(std::string_view key) const {
...
KeyEpochDecision merge_key_epoch_decisions(const std::vector<KeyEpochDecision>& decisions) {
    KeyEpochDecision merged;
    if (decisions.empty()) {
        merged.reason = "no key epochs";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "epoch selection" : "pending evidence";
    return merged;
}

std::string render_key_epoch_decision(const KeyEpochDecision& decision) {
    std::ostringstream out;
    out << "KeyEpoch(accepted=" << (decision.accepted ? "true" : "false")
