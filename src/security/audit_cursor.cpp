...
AuditCursor::AuditCursor(std::string owner) : owner_(std::move(owner)) {}

void AuditCursor::insert(AuditCursorEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void AuditCursor::remove_before(std::uint64_t tick) {
...
AuditCursorDecision AuditCursor::evaluate(std::string_view key, double threshold) const {
    AuditCursorDecision decision;
    double total = 0.0;
...
std::vector<AuditCursorEntry> AuditCursor::drain(double minimum_score) const {
    std::vector<AuditCursorEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<AuditCursorEntry> AuditCursor::find(std::string_view key) const {
...
AuditCursorDecision merge_audit_cursor_decisions(const std::vector<AuditCursorDecision>& decisions) {
    AuditCursorDecision merged;
    if (decisions.empty()) {
        merged.reason = "no audit offsets";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "cursor tracking" : "pending evidence";
    return merged;
}

std::string render_audit_cursor_decision(const AuditCursorDecision& decision) {
    std::ostringstream out;
    out << "AuditCursor(accepted=" << (decision.accepted ? "true" : "false")
