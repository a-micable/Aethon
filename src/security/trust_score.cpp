...
TrustScore::TrustScore(std::string owner) : owner_(std::move(owner)) {}

void TrustScore::insert(TrustScoreEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void TrustScore::remove_before(std::uint64_t tick) {
...
TrustScoreDecision TrustScore::evaluate(std::string_view key, double threshold) const {
    TrustScoreDecision decision;
    double total = 0.0;
...
std::vector<TrustScoreEntry> TrustScore::drain(double minimum_score) const {
    std::vector<TrustScoreEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<TrustScoreEntry> TrustScore::find(std::string_view key) const {
...
TrustScoreDecision merge_trust_score_decisions(const std::vector<TrustScoreDecision>& decisions) {
    TrustScoreDecision merged;
    if (decisions.empty()) {
        merged.reason = "no trust samples";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "trust scoring" : "pending evidence";
    return merged;
}

std::string render_trust_score_decision(const TrustScoreDecision& decision) {
    std::ostringstream out;
    out << "TrustScore(accepted=" << (decision.accepted ? "true" : "false")
