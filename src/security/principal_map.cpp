#include "aethon/security/principal_map.hpp"
...
PrincipalMap::PrincipalMap(std::string owner) : owner_(std::move(owner)) {}

void PrincipalMap::insert(PrincipalMapEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void PrincipalMap::remove_before(std::uint64_t tick) {
...
PrincipalMapDecision PrincipalMap::evaluate(std::string_view key, double threshold) const {
    PrincipalMapDecision decision;
    double total = 0.0;
...
std::vector<PrincipalMapEntry> PrincipalMap::drain(double minimum_score) const {
    std::vector<PrincipalMapEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<PrincipalMapEntry> PrincipalMap::find(std::string_view key) const {
...
PrincipalMapDecision merge_principal_map_decisions(const std::vector<PrincipalMapDecision>& decisions) {
    PrincipalMapDecision merged;
    if (decisions.empty()) {
        merged.reason = "no principals";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "principal lookup" : "pending evidence";
    return merged;
}

std::string render_principal_map_decision(const PrincipalMapDecision& decision) {
    std::ostringstream out;
    out << "PrincipalMap(accepted=" << (decision.accepted ? "true" : "false")
