...
QuarantineSet::QuarantineSet(std::string owner) : owner_(std::move(owner)) {}

void QuarantineSet::insert(QuarantineSetEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void QuarantineSet::remove_before(std::uint64_t tick) {
...
QuarantineSetDecision QuarantineSet::evaluate(std::string_view key, double threshold) const {
    QuarantineSetDecision decision;
    double total = 0.0;
...
std::vector<QuarantineSetEntry> QuarantineSet::drain(double minimum_score) const {
    std::vector<QuarantineSetEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<QuarantineSetEntry> QuarantineSet::find(std::string_view key) const {
...
QuarantineSetDecision merge_quarantine_set_decisions(const std::vector<QuarantineSetDecision>& decisions) {
    QuarantineSetDecision merged;
    if (decisions.empty()) {
        merged.reason = "no quarantine entries";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "quarantine decisions" : "pending evidence";
    return merged;
}

std::string render_quarantine_set_decision(const QuarantineSetDecision& decision) {
    std::ostringstream out;
    out << "QuarantineSet(accepted=" << (decision.accepted ? "true" : "false")
