...
TokenBucket::TokenBucket(std::string owner) : owner_(std::move(owner)) {}

void TokenBucket::insert(TokenBucketEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void TokenBucket::remove_before(std::uint64_t tick) {
...
TokenBucketDecision TokenBucket::evaluate(std::string_view key, double threshold) const {
...
std::vector<TokenBucketEntry> TokenBucket::drain(double minimum_score) const {
    std::vector<TokenBucketEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<TokenBucketEntry> TokenBucket::find(std::string_view key) const {
...
TokenBucketDecision merge_token_bucket_decisions(const std::vector<TokenBucketDecision>& decisions) {
    TokenBucketDecision merged;
    if (decisions.empty()) {
        merged.reason = "no tokens";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "token accounting" : "pending evidence";
    return merged;
}

std::string render_token_bucket_decision(const TokenBucketDecision& decision) {
    std::ostringstream out;
    out << "TokenBucket(accepted=" << (decision.accepted ? "true" : "false")
