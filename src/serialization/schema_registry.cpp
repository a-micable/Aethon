...
SchemaRegistry::SchemaRegistry(std::string name) : name_(std::move(name)) {}

void SchemaRegistry::observe(SchemaRegistrySample sample) {
...
void SchemaRegistry::clear_before(std::uint64_t timestamp_ns) {
...
SchemaRegistrySummary SchemaRegistry::summarize() const {
    std::vector<SchemaRegistrySample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_schema_registry(copy);
...
std::optional<SchemaRegistrySample> SchemaRegistry::latest(std::string_view label) const {
...
std::vector<SchemaRegistrySample> SchemaRegistry::select(double threshold) const {
    std::vector<SchemaRegistrySample> out;
...
SchemaRegistrySummary summarize_schema_registry(const std::vector<SchemaRegistrySample>& samples) {
    SchemaRegistrySummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no schema versions observed";
        return summary;
    }
...
std::string describe_schema_registry(const SchemaRegistrySummary& summary) {
    ...
    out << "SchemaRegistry[count=" << summary.count
        << ", age=" << summary.average
