...
RecordBuilder::RecordBuilder(std::string name) : name_(std::move(name)) {}

void RecordBuilder::observe(RecordBuilderSample sample) {
...
void RecordBuilder::clear_before(std::uint64_t timestamp_ns) {
...
RecordBuilderSummary RecordBuilder::summarize() const {
    std::vector<RecordBuilderSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_record_builder(copy);
...
std::optional<RecordBuilderSample> RecordBuilder::latest(std::string_view label) const {
...
std::vector<RecordBuilderSample> RecordBuilder::select(double threshold) const {
    std::vector<RecordBuilderSample> out;
...
RecordBuilderSummary summarize_record_builder(const std::vector<RecordBuilderSample>& samples) {
    RecordBuilderSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no structured records observed";
        return summary;
    }
    ...
    summary.note = "stable binary layouts";
    return summary;
}
...
std::string describe_record_builder(const RecordBuilderSummary& summary) {
    ...
    out << "RecordBuilder[count=" << summary.count
