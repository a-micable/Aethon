...
BlobStore::BlobStore(std::string name) : name_(std::move(name)) {}

void BlobStore::observe(BlobStoreSample sample) {
...
void BlobStore::clear_before(std::uint64_t timestamp_ns) {
...
BlobStoreSummary BlobStore::summarize() const {
    std::vector<BlobStoreSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_blob_store(copy);
...
std::optional<BlobStoreSample> BlobStore::latest(std::string_view label) const {
...
std::vector<BlobStoreSample> BlobStore::select(double threshold) const {
    std::vector<BlobStoreSample> out;
...
BlobStoreSummary summarize_blob_store(const std::vector<BlobStoreSample>& samples) {
    ...
    if (samples.empty()) {
        summary.note = "no opaque blobs observed";
        return summary;
    }
    ...
    summary.note = "deduplicated payload storage";
    return summary;
}
...
std::string describe_blob_store(const BlobStoreSummary& summary) {
    ...
    out << "BlobStore[count=" << summary.count
