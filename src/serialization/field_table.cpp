#include "aethon/serialization/field_table.hpp"
...
FieldTable::FieldTable(std::string name) : name_(std::move(name)) {}

void FieldTable::observe(FieldTableSample sample) {
...
void FieldTable::clear_before(std::uint64_t timestamp_ns) {
...
FieldTableSummary FieldTable::summarize() const {
    std::vector<FieldTableSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_field_table(copy);
...
std::optional<FieldTableSample> FieldTable::latest(std::string_view label) const {
...
std::vector<FieldTableSample> FieldTable::select(double threshold) const {
    std::vector<FieldTableSample> out;
...
FieldTableSummary summarize_field_table(const std::vector<FieldTableSample>& samples) {
    FieldTableSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no binary fields observed";
        return summary;
    }
    ...
    summary.note = "schema field ordering";
    return summary;
}
...
std::string describe_field_table(const FieldTableSummary& summary) {
    ...
    out << "FieldTable[count=" << summary.count
