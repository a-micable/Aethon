#include "aethon/protocol/capability_set.hpp"
...
CapabilitySet::CapabilitySet(std::string name) : name_(std::move(name)) {}

void CapabilitySet::observe(CapabilitySetSample sample) {
...
void CapabilitySet::clear_before(std::uint64_t timestamp_ns) {
...
CapabilitySetSummary CapabilitySet::summarize() const {
    std::vector<CapabilitySetSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_capability_set(copy);
...
std::optional<CapabilitySetSample> CapabilitySet::latest(std::string_view label) const {
...
std::vector<CapabilitySetSample> CapabilitySet::select(double threshold) const {
    std::vector<CapabilitySetSample> out;
...
CapabilitySetSummary summarize_capability_set(const std::vector<CapabilitySetSample>& samples) {
    CapabilitySetSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no device capabilities observed";
        return summary;
    }
    ...
    summary.note = "feature negotiation";
    return summary;
}
...
std::string describe_capability_set(const CapabilitySetSummary& summary) {
    ...
    out << "CapabilitySet[count=" << summary.count
