...
ExtensionRegistry::ExtensionRegistry(std::string name) : name_(std::move(name)) {}

void ExtensionRegistry::observe(ExtensionRegistrySample sample) {
...
void ExtensionRegistry::clear_before(std::uint64_t timestamp_ns) {
...
ExtensionRegistrySummary ExtensionRegistry::summarize() const {
    std::vector<ExtensionRegistrySample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_extension_registry(copy);
...
std::optional<ExtensionRegistrySample> ExtensionRegistry::latest(std::string_view label) const {
...
std::vector<ExtensionRegistrySample> ExtensionRegistry::select(double threshold) const {
    std::vector<ExtensionRegistrySample> out;
...
ExtensionRegistrySummary summarize_extension_registry(const std::vector<ExtensionRegistrySample>& samples) {
    ExtensionRegistrySummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no extension headers observed";
        return summary;
    }
...
std::string describe_extension_registry(const ExtensionRegistrySummary& summary) {
    ...
    out << "ExtensionRegistry[count=" << summary.count
