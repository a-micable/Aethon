#include "aethon/protocol/metadata_view.hpp"
...
MetadataView::MetadataView(std::string name) : name_(std::move(name)) {}

void MetadataView::observe(MetadataViewSample sample) {
...
void MetadataView::clear_before(std::uint64_t timestamp_ns) {
...
MetadataViewSummary MetadataView::summarize() const {
    std::vector<MetadataViewSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_metadata_view(copy);
...
std::optional<MetadataViewSample> MetadataView::latest(std::string_view label) const {
...
std::vector<MetadataViewSample> MetadataView::select(double threshold) const {
    std::vector<MetadataViewSample> out;
...
MetadataViewSummary summarize_metadata_view(const std::vector<MetadataViewSample>& samples) {
    MetadataViewSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no packet metadata observed";
        return summary;
    }
...
std::string describe_metadata_view(const MetadataViewSummary& summary) {
    ...
    out << "MetadataView[count=" << summary.count
