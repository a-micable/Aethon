...
VersionMatrix::VersionMatrix(std::string name) : name_(std::move(name)) {}

void VersionMatrix::observe(VersionMatrixSample sample) {
...
void VersionMatrix::clear_before(std::uint64_t timestamp_ns) {
...
VersionMatrixSummary VersionMatrix::summarize() const {
    std::vector<VersionMatrixSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_version_matrix(copy);
...
std::optional<VersionMatrixSample> VersionMatrix::latest(std::string_view label) const {
...
std::vector<VersionMatrixSample> VersionMatrix::select(double threshold) const {
    std::vector<VersionMatrixSample> out;
...
VersionMatrixSummary summarize_version_matrix(const std::vector<VersionMatrixSample>& samples) {
    VersionMatrixSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no firmware versions observed";
        return summary;
    }
    ...
    summary.note = "compatibility decisions";
    return summary;
}

double version_matrix_stability_index(const VersionMatrixSummary& summary) {
    if (summary.count == 0) {
        return 0.0;
    }
...
std::string describe_version_matrix(const VersionMatrixSummary& summary) {
    std::ostringstream out;
    out << "VersionMatrix[count=" << summary.count
        << ", ratio=" << summary.average
