#include "aethon/protocol/fragment_map.hpp"
...
FragmentMap::FragmentMap(std::string name) : name_(std::move(name)) {}

void FragmentMap::observe(FragmentMapSample sample) {
...
void FragmentMap::clear_before(std::uint64_t timestamp_ns) {
...
FragmentMapSummary FragmentMap::summarize() const {
    std::vector<FragmentMapSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_fragment_map(copy);
...
std::optional<FragmentMapSample> FragmentMap::latest(std::string_view label) const {
...
std::vector<FragmentMapSample> FragmentMap::select(double threshold) const {
    std::vector<FragmentMapSample> out;
...
FragmentMapSummary summarize_fragment_map(const std::vector<FragmentMapSample>& samples) {
    FragmentMapSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no message fragments observed";
        return summary;
    }
    ...
    summary.note = "completion tracking";
    return summary;
}
...
std::string describe_fragment_map(const FragmentMapSummary& summary) {
    ...
    out << "FragmentMap[count=" << summary.count
