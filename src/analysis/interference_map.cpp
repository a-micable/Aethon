...
InterferenceMap::InterferenceMap(std::string name) : name_(std::move(name)) {}

void InterferenceMap::observe(InterferenceMapSample sample) {
...
void InterferenceMap::clear_before(std::uint64_t timestamp_ns) {
...
InterferenceMapSummary InterferenceMap::summarize() const {
    std::vector<InterferenceMapSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_interference_map(copy);
...
std::optional<InterferenceMapSample> InterferenceMap::latest(std::string_view label) const {
...
std::vector<InterferenceMapSample> InterferenceMap::select(double threshold) const {
    std::vector<InterferenceMapSample> out;
...
InterferenceMapSummary summarize_interference_map(const std::vector<InterferenceMapSample>& samples) {
    InterferenceMapSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no regional observations observed";
        return summary;
    }
    ...
    summary.note = "interference heat scores";
    return summary;
}
...
std::string describe_interference_map(const InterferenceMapSummary& summary) {
    ...
    out << "InterferenceMap[count=" << summary.count
