#include "aethon/analysis/iq_statistics.hpp"
...
IqStatistics::IqStatistics(std::string name) : name_(std::move(name)) {}

void IqStatistics::observe(IqStatisticsSample sample) {
...
void IqStatistics::clear_before(std::uint64_t timestamp_ns) {
...
IqStatisticsSummary IqStatistics::summarize() const {
    std::vector<IqStatisticsSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_iq_statistics(copy);
...
std::optional<IqStatisticsSample> IqStatistics::latest(std::string_view label) const {
...
std::vector<IqStatisticsSample> IqStatistics::select(double threshold) const {
    std::vector<IqStatisticsSample> out;
...
IqStatisticsSummary summarize_iq_statistics(const std::vector<IqStatisticsSample>& samples) {
    IqStatisticsSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no complex sample batches observed";
        return summary;
    }
    ...
    summary.note = "phase and amplitude summaries";
    return summary;
}
...
std::string describe_iq_statistics(const IqStatisticsSummary& summary) {
    ...
    out << "IqStatistics[count=" << summary.count
