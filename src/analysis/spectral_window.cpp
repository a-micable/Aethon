...
SpectralWindow::SpectralWindow(std::string name) : name_(std::move(name)) {}

void SpectralWindow::observe(SpectralWindowSample sample) {
...
void SpectralWindow::clear_before(std::uint64_t timestamp_ns) {
...
SpectralWindowSummary SpectralWindow::summarize() const {
    std::vector<SpectralWindowSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_spectral_window(copy);
...
std::optional<SpectralWindowSample> SpectralWindow::latest(std::string_view label) const {
...
std::vector<SpectralWindowSample> SpectralWindow::select(double threshold) const {
    std::vector<SpectralWindowSample> out;
...
SpectralWindowSummary summarize_spectral_window(const std::vector<SpectralWindowSample>& samples) {
    SpectralWindowSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no frequency bins observed";
        return summary;
    }
    ...
    summary.note = "windowed energy drift";
    return summary;
}
...
std::string describe_spectral_window(const SpectralWindowSummary& summary) {
    ...
    out << "SpectralWindow[count=" << summary.count
