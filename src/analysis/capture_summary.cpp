...
CaptureSummary::CaptureSummary(std::string name) : name_(std::move(name)) {}

void CaptureSummary::observe(CaptureSummarySample sample) {
...
void CaptureSummary::clear_before(std::uint64_t timestamp_ns) {
...
CaptureSummarySummary CaptureSummary::summarize() const {
    std::vector<CaptureSummarySample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_capture_summary(copy);
...
std::optional<CaptureSummarySample> CaptureSummary::latest(std::string_view label) const {
...
std::vector<CaptureSummarySample> CaptureSummary::select(double threshold) const {
    std::vector<CaptureSummarySample> out;
...
CaptureSummarySummary summarize_capture_summary(const std::vector<CaptureSummarySample>& samples) {
    CaptureSummarySummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no archive captures observed";
        return summary;
    }
...
std::string describe_capture_summary(const CaptureSummarySummary& summary) {
    ...
    out << "CaptureSummary[count=" << summary.count
