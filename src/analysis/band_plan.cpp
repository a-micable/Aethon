...
BandPlan::BandPlan(std::string name) : name_(std::move(name)) {}

void BandPlan::observe(BandPlanSample sample) {
...
void BandPlan::clear_before(std::uint64_t timestamp_ns) {
...
BandPlanSummary BandPlan::summarize() const {
    std::vector<BandPlanSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_band_plan(copy);
...
std::optional<BandPlanSample> BandPlan::latest(std::string_view label) const {
...
std::vector<BandPlanSample> BandPlan::select(double threshold) const {
    std::vector<BandPlanSample> out;
...
BandPlanSummary summarize_band_plan(const std::vector<BandPlanSample>& samples) {
    BandPlanSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no regulatory bands observed";
        return summary;
    }
    ...
    summary.note = "frequency containment checks";
    return summary;
}
...
std::string describe_band_plan(const BandPlanSummary& summary) {
    ...
    out << "BandPlan[count=" << summary.count
