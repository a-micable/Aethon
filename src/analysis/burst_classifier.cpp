#include "aethon/analysis/burst_classifier.hpp"
...
BurstClassifier::BurstClassifier(std::string name) : name_(std::move(name)) {}

void BurstClassifier::observe(BurstClassifierSample sample) {
...
void BurstClassifier::clear_before(std::uint64_t timestamp_ns) {
...
BurstClassifierSummary BurstClassifier::summarize() const {
    std::vector<BurstClassifierSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_burst_classifier(copy);
...
std::optional<BurstClassifierSample> BurstClassifier::latest(std::string_view label) const {
...
std::vector<BurstClassifierSample> BurstClassifier::select(double threshold) const {
    std::vector<BurstClassifierSample> out;
...
BurstClassifierSummary summarize_burst_classifier(const std::vector<BurstClassifierSample>& samples) {
    BurstClassifierSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no short RF bursts observed";
        return summary;
    }
    ...
    summary.note = "classification confidence";
    return summary;
}
...
std::string describe_burst_classifier(const BurstClassifierSummary& summary) {
    ...
    out << "BurstClassifier[count=" << summary.count
