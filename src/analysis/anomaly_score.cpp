#include "aethon/analysis/anomaly_score.hpp"
...
AnomalyScore::AnomalyScore(std::string name) : name_(std::move(name)) {}

void AnomalyScore::observe(AnomalyScoreSample sample) {
...
void AnomalyScore::clear_before(std::uint64_t timestamp_ns) {
...
AnomalyScoreSummary AnomalyScore::summarize() const {
    std::vector<AnomalyScoreSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_anomaly_score(copy);
...
std::optional<AnomalyScoreSample> AnomalyScore::latest(std::string_view label) const {
...
std::vector<AnomalyScoreSample> AnomalyScore::select(double threshold) const {
    std::vector<AnomalyScoreSample> out;
...
AnomalyScoreSummary summarize_anomaly_score(const std::vector<AnomalyScoreSample>& samples) {
    AnomalyScoreSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no telemetry observations observed";
        return summary;
    }
    ...
    summary.note = "robust anomaly ranking";
    return summary;
}
...
std::string describe_anomaly_score(const AnomalyScoreSummary& summary) {
    ...
    out << "AnomalyScore[count=" << summary.count
