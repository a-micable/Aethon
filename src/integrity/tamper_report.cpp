...
TamperReport::TamperReport(std::string name) : name_(std::move(name)) {}

void TamperReport::observe(TamperReportSample sample) {
...
void TamperReport::clear_before(std::uint64_t timestamp_ns) {
...
TamperReportSummary TamperReport::summarize() const {
    std::vector<TamperReportSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_tamper_report(copy);
...
std::optional<TamperReportSample> TamperReport::latest(std::string_view label) const {
...
std::vector<TamperReportSample> TamperReport::select(double threshold) const {
    std::vector<TamperReportSample> out;
...
TamperReportSummary summarize_tamper_report(const std::vector<TamperReportSample>& samples) {
    TamperReportSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no failed records observed";
        return summary;
    }
    ...
    summary.note = "tamper evidence summaries";
    return summary;
}
...
std::string describe_tamper_report(const TamperReportSummary& summary) {
    ...
    out << "TamperReport[count=" << summary.count
