...
SequenceTracker::SequenceTracker(std::string name) : name_(std::move(name)) {}

void SequenceTracker::observe(SequenceTrackerSample sample) {
...
void SequenceTracker::clear_before(std::uint64_t timestamp_ns) {
...
SequenceTrackerSummary SequenceTracker::summarize() const {
    std::vector<SequenceTrackerSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_sequence_tracker(copy);
...
std::optional<SequenceTrackerSample> SequenceTracker::latest(std::string_view label) const {
...
std::vector<SequenceTrackerSample> SequenceTracker::select(double threshold) const {
    std::vector<SequenceTrackerSample> out;
...
SequenceTrackerSummary summarize_sequence_tracker(const std::vector<SequenceTrackerSample>& samples) {
    SequenceTrackerSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no packet sequences observed";
        return summary;
    }
...
std::string describe_sequence_tracker(const SequenceTrackerSummary& summary) {
    ...
    out << "SequenceTracker[count=" << summary.count
