...
DigestCache::DigestCache(std::string name) : name_(std::move(name)) {}

void DigestCache::observe(DigestCacheSample sample) {
...
void DigestCache::clear_before(std::uint64_t timestamp_ns) {
...
DigestCacheSummary DigestCache::summarize() const {
    std::vector<DigestCacheSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_digest_cache(copy);
...
std::optional<DigestCacheSample> DigestCache::latest(std::string_view label) const {
...
std::vector<DigestCacheSample> DigestCache::select(double threshold) const {
    std::vector<DigestCacheSample> out;
...
DigestCacheSummary summarize_digest_cache(const std::vector<DigestCacheSample>& samples) {
    DigestCacheSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no packet digests observed";
        return summary;
    }
    ...
    summary.note = "recent digest reuse";
    return summary;
}
...
std::string describe_digest_cache(const DigestCacheSummary& summary) {
    ...
    out << "DigestCache[count=" << summary.count
