#include "aethon/integrity/verification_plan.hpp"
...
VerificationPlan::VerificationPlan(std::string name) : name_(std::move(name)) {}

void VerificationPlan::observe(VerificationPlanSample sample) {
...
void VerificationPlan::clear_before(std::uint64_t timestamp_ns) {
...
VerificationPlanSummary VerificationPlan::summarize() const {
    std::vector<VerificationPlanSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_verification_plan(copy);
...
std::optional<VerificationPlanSample> VerificationPlan::latest(std::string_view label) const {
...
std::vector<VerificationPlanSample> VerificationPlan::select(double threshold) const {
    std::vector<VerificationPlanSample> out;
...
VerificationPlanSummary summarize_verification_plan(const std::vector<VerificationPlanSample>& samples) {
    VerificationPlanSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no checksums observed";
        return summary;
    }
    ...
    summary.note = "multi-stage verification";
    return summary;
}
...
std::string describe_verification_plan(const VerificationPlanSummary& summary) {
    ...
    out << "VerificationPlan[count=" << summary.count
