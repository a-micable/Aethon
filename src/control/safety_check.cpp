...
SafetyCheck::SafetyCheck(std::string owner) : owner_(std::move(owner)) {}

void SafetyCheck::insert(SafetyCheckEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void SafetyCheck::remove_before(std::uint64_t tick) {
...
SafetyCheckDecision SafetyCheck::evaluate(std::string_view key, double threshold) const {
    SafetyCheckDecision decision;
    double total = 0.0;
...
std::vector<SafetyCheckEntry> SafetyCheck::drain(double minimum_score) const {
    std::vector<SafetyCheckEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<SafetyCheckEntry> SafetyCheck::find(std::string_view key) const {
...
SafetyCheckDecision merge_safety_check_decisions(const std::vector<SafetyCheckDecision>& decisions) {
    SafetyCheckDecision merged;
    if (decisions.empty()) {
        merged.reason = "no control checks";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "safety scoring" : "pending evidence";
    return merged;
}

std::string render_safety_check_decision(const SafetyCheckDecision& decision) {
    std::ostringstream out;
    out << "SafetyCheck(accepted=" << (decision.accepted ? "true" : "false")
