...
AckTracker::AckTracker(std::string owner) : owner_(std::move(owner)) {}

void AckTracker::insert(AckTrackerEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void AckTracker::remove_before(std::uint64_t tick) {
...
AckTrackerDecision AckTracker::evaluate(std::string_view key, double threshold) const {
    AckTrackerDecision decision;
    double total = 0.0;
...
std::vector<AckTrackerEntry> AckTracker::drain(double minimum_score) const {
    std::vector<AckTrackerEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<AckTrackerEntry> AckTracker::find(std::string_view key) const {
...
AckTrackerDecision merge_ack_tracker_decisions(const std::vector<AckTrackerDecision>& decisions) {
    AckTrackerDecision merged;
    if (decisions.empty()) {
        merged.reason = "no acknowledgements";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "ack latency" : "pending evidence";
    return merged;
}

std::string render_ack_tracker_decision(const AckTrackerDecision& decision) {
    std::ostringstream out;
    out << "AckTracker(accepted=" << (decision.accepted ? "true" : "false")
