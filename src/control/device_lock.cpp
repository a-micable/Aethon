...
DeviceLock::DeviceLock(std::string owner) : owner_(std::move(owner)) {}

void DeviceLock::insert(DeviceLockEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void DeviceLock::remove_before(std::uint64_t tick) {
...
DeviceLockDecision DeviceLock::evaluate(std::string_view key, double threshold) const {
...
std::vector<DeviceLockEntry> DeviceLock::drain(double minimum_score) const {
    std::vector<DeviceLockEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<DeviceLockEntry> DeviceLock::find(std::string_view key) const {
...
DeviceLockDecision merge_device_lock_decisions(const std::vector<DeviceLockDecision>& decisions) {
    DeviceLockDecision merged;
    if (decisions.empty()) {
        merged.reason = "no device locks";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "lock ownership" : "pending evidence";
    return merged;
}

std::string render_device_lock_decision(const DeviceLockDecision& decision) {
    std::ostringstream out;
    out << "DeviceLock(accepted=" << (decision.accepted ? "true" : "false")
