...
CommandQueue::CommandQueue(std::string owner) : owner_(std::move(owner)) {}

void CommandQueue::insert(CommandQueueEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void CommandQueue::remove_before(std::uint64_t tick) {
...
CommandQueueDecision CommandQueue::evaluate(std::string_view key, double threshold) const {
    CommandQueueDecision decision;
    double total = 0.0;
...
std::vector<CommandQueueEntry> CommandQueue::drain(double minimum_score) const {
    std::vector<CommandQueueEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<CommandQueueEntry> CommandQueue::find(std::string_view key) const {
...
CommandQueueDecision merge_command_queue_decisions(const std::vector<CommandQueueDecision>& decisions) {
    CommandQueueDecision merged;
    if (decisions.empty()) {
        merged.reason = "no control commands";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "queue ordering" : "pending evidence";
    return merged;
}

std::string render_command_queue_decision(const CommandQueueDecision& decision) {
    std::ostringstream out;
    out << "CommandQueue(accepted=" << (decision.accepted ? "true" : "false")
