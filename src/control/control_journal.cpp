#include "aethon/control/control_journal.hpp"
...
ControlJournal::ControlJournal(std::string owner) : owner_(std::move(owner)) {}

void ControlJournal::insert(ControlJournalEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void ControlJournal::remove_before(std::uint64_t tick) {
...
ControlJournalDecision ControlJournal::evaluate(std::string_view key, double threshold) const {
    ControlJournalDecision decision;
    double total = 0.0;
...
std::vector<ControlJournalEntry> ControlJournal::drain(double minimum_score) const {
    std::vector<ControlJournalEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<ControlJournalEntry> ControlJournal::find(std::string_view key) const {
...
ControlJournalDecision merge_control_journal_decisions(const std::vector<ControlJournalDecision>& decisions) {
    ControlJournalDecision merged;
    if (decisions.empty()) {
        merged.reason = "no control events";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "journal summaries" : "pending evidence";
    return merged;
}

std::string render_control_journal_decision(const ControlJournalDecision& decision) {
    std::ostringstream out;
    out << "ControlJournal(accepted=" << (decision.accepted ? "true" : "false")
