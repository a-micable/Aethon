...
FirmwareRollout::FirmwareRollout(std::string owner) : owner_(std::move(owner)) {}

void FirmwareRollout::insert(FirmwareRolloutEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void FirmwareRollout::remove_before(std::uint64_t tick) {
...
FirmwareRolloutDecision FirmwareRollout::evaluate(std::string_view key, double threshold) const {
    FirmwareRolloutDecision decision;
    double total = 0.0;
...
std::vector<FirmwareRolloutEntry> FirmwareRollout::drain(double minimum_score) const {
    std::vector<FirmwareRolloutEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<FirmwareRolloutEntry> FirmwareRollout::find(std::string_view key) const {
...
FirmwareRolloutDecision merge_firmware_rollout_decisions(const std::vector<FirmwareRolloutDecision>& decisions) {
    FirmwareRolloutDecision merged;
    if (decisions.empty()) {
        merged.reason = "no rollout steps";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "rollout safety" : "pending evidence";
    return merged;
}

std::string render_firmware_rollout_decision(const FirmwareRolloutDecision& decision) {
    std::ostringstream out;
    out << "FirmwareRollout(accepted=" << (decision.accepted ? "true" : "false")
