...
RateLimit::RateLimit(std::string owner) : owner_(std::move(owner)) {}

void RateLimit::insert(RateLimitEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void RateLimit::remove_before(std::uint64_t tick) {
...
RateLimitDecision RateLimit::evaluate(std::string_view key, double threshold) const {
...
std::vector<RateLimitEntry> RateLimit::drain(double minimum_score) const {
    std::vector<RateLimitEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<RateLimitEntry> RateLimit::find(std::string_view key) const {
...
RateLimitDecision merge_rate_limit_decisions(const std::vector<RateLimitDecision>& decisions) {
    RateLimitDecision merged;
    if (decisions.empty()) {
        merged.reason = "no command rates";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "rate enforcement" : "pending evidence";
    return merged;
}

std::string render_rate_limit_decision(const RateLimitDecision& decision) {
    std::ostringstream out;
    out << "RateLimit(accepted=" << (decision.accepted ? "true" : "false")
