#include "aethon/control/maintenance_window.hpp"
...
MaintenanceWindow::MaintenanceWindow(std::string owner) : owner_(std::move(owner)) {}

void MaintenanceWindow::insert(MaintenanceWindowEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void MaintenanceWindow::remove_before(std::uint64_t tick) {
...
MaintenanceWindowDecision MaintenanceWindow::evaluate(std::string_view key, double threshold) const {
    MaintenanceWindowDecision decision;
    double total = 0.0;
...
std::vector<MaintenanceWindowEntry> MaintenanceWindow::drain(double minimum_score) const {
    std::vector<MaintenanceWindowEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<MaintenanceWindowEntry> MaintenanceWindow::find(std::string_view key) const {
...
MaintenanceWindowDecision merge_maintenance_window_decisions(const std::vector<MaintenanceWindowDecision>& decisions) {
    MaintenanceWindowDecision merged;
    if (decisions.empty()) {
        merged.reason = "no maintenance windows";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "window eligibility" : "pending evidence";
    return merged;
}

std::string render_maintenance_window_decision(const MaintenanceWindowDecision& decision) {
    std::ostringstream out;
    out << "MaintenanceWindow(accepted=" << (decision.accepted ? "true" : "false")
