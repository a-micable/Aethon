...
RollbackPlan::RollbackPlan(std::string owner) : owner_(std::move(owner)) {}

void RollbackPlan::insert(RollbackPlanEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void RollbackPlan::remove_before(std::uint64_t tick) {
...
RollbackPlanDecision RollbackPlan::evaluate(std::string_view key, double threshold) const {
    RollbackPlanDecision decision;
    double total = 0.0;
...
std::vector<RollbackPlanEntry> RollbackPlan::drain(double minimum_score) const {
    std::vector<RollbackPlanEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<RollbackPlanEntry> RollbackPlan::find(std::string_view key) const {
...
RollbackPlanDecision merge_rollback_plan_decisions(const std::vector<RollbackPlanDecision>& decisions) {
    RollbackPlanDecision merged;
    if (decisions.empty()) {
        merged.reason = "no rollback steps";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "rollback readiness" : "pending evidence";
    return merged;
}

std::string render_rollback_plan_decision(const RollbackPlanDecision& decision) {
    std::ostringstream out;
    out << "RollbackPlan(accepted=" << (decision.accepted ? "true" : "false")
