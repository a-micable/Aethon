...
OperatorIntent::OperatorIntent(std::string owner) : owner_(std::move(owner)) {}

void OperatorIntent::insert(OperatorIntentEntry entry) {
    if (entry.key.empty()) {
        entry.key = "default";
    }
...
void OperatorIntent::remove_before(std::uint64_t tick) {
...
OperatorIntentDecision OperatorIntent::evaluate(std::string_view key, double threshold) const {
    OperatorIntentDecision decision;
    double total = 0.0;
...
std::vector<OperatorIntentEntry> OperatorIntent::drain(double minimum_score) const {
    std::vector<OperatorIntentEntry> out;
    for (const auto& entry : entries_) {
...
std::optional<OperatorIntentEntry> OperatorIntent::find(std::string_view key) const {
...
OperatorIntentDecision merge_operator_intent_decisions(const std::vector<OperatorIntentDecision>& decisions) {
    OperatorIntentDecision merged;
    if (decisions.empty()) {
        merged.reason = "no operator intents";
        return merged;
    }
    ...
    merged.reason = merged.accepted ? "intent filtering" : "pending evidence";
    return merged;
}

std::string render_operator_intent_decision(const OperatorIntentDecision& decision) {
    std::ostringstream out;
    out << "OperatorIntent(accepted=" << (decision.accepted ? "true" : "false")
