...
IncidentMarker::IncidentMarker(std::string name) : name_(std::move(name)) {}

void IncidentMarker::observe(IncidentMarkerSample sample) {
...
void IncidentMarker::clear_before(std::uint64_t timestamp_ns) {
...
IncidentMarkerSummary IncidentMarker::summarize() const {
    std::vector<IncidentMarkerSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_incident_marker(copy);
...
std::optional<IncidentMarkerSample> IncidentMarker::latest(std::string_view label) const {
...
std::vector<IncidentMarkerSample> IncidentMarker::select(double threshold) const {
    std::vector<IncidentMarkerSample> out;
...
IncidentMarkerSummary summarize_incident_marker(const std::vector<IncidentMarkerSample>& samples) {
    IncidentMarkerSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no operator events observed";
        return summary;
    }
    ...
    summary.note = "incident annotations";
    return summary;
}
...
std::string describe_incident_marker(const IncidentMarkerSummary& summary) {
    ...
    out << "IncidentMarker[count=" << summary.count
