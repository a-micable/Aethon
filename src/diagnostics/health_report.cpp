#include "aethon/diagnostics/health_report.hpp"
...
HealthReport::HealthReport(std::string name) : name_(std::move(name)) {}

void HealthReport::observe(HealthReportSample sample) {
...
void HealthReport::clear_before(std::uint64_t timestamp_ns) {
...
HealthReportSummary HealthReport::summarize() const {
    std::vector<HealthReportSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_health_report(copy);
...
std::optional<HealthReportSample> HealthReport::latest(std::string_view label) const {
...
std::vector<HealthReportSample> HealthReport::select(double threshold) const {
    std::vector<HealthReportSample> out;
...
HealthReportSummary summarize_health_report(const std::vector<HealthReportSample>& samples) {
    ...
    if (samples.empty()) {
        summary.note = "no component states observed";
        return summary;
    }
    ...
    summary.note = "service health rollups";
    return summary;
}
...
std::string describe_health_report(const HealthReportSummary& summary) {
    ...
    out << "HealthReport[count=" << summary.count
