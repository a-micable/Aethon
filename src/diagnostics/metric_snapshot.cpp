#include "aethon/diagnostics/metric_snapshot.hpp"
...
MetricSnapshot::MetricSnapshot(std::string name) : name_(std::move(name)) {}

void MetricSnapshot::observe(MetricSnapshotSample sample) {
...
void MetricSnapshot::clear_before(std::uint64_t timestamp_ns) {
...
MetricSnapshotSummary MetricSnapshot::summarize() const {
    std::vector<MetricSnapshotSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_metric_snapshot(copy);
...
std::optional<MetricSnapshotSample> MetricSnapshot::latest(std::string_view label) const {
...
std::vector<MetricSnapshotSample> MetricSnapshot::select(double threshold) const {
    std::vector<MetricSnapshotSample> out;
...
MetricSnapshotSummary summarize_metric_snapshot(const std::vector<MetricSnapshotSample>& samples) {
    ...
    if (samples.empty()) {
        summary.note = "no counter samples observed";
        return summary;
    }
    ...
    summary.note = "point-in-time export";
    return summary;
}
...
std::string describe_metric_snapshot(const MetricSnapshotSummary& summary) {
    ...
    out << "MetricSnapshot[count=" << summary.count
