...
TraceBuffer::TraceBuffer(std::string name) : name_(std::move(name)) {}

void TraceBuffer::observe(TraceBufferSample sample) {
...
void TraceBuffer::clear_before(std::uint64_t timestamp_ns) {
...
TraceBufferSummary TraceBuffer::summarize() const {
    std::vector<TraceBufferSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_trace_buffer(copy);
...
std::optional<TraceBufferSample> TraceBuffer::latest(std::string_view label) const {
...
std::vector<TraceBufferSample> TraceBuffer::select(double threshold) const {
    std::vector<TraceBufferSample> out;
...
TraceBufferSummary summarize_trace_buffer(const std::vector<TraceBufferSample>& samples) {
    TraceBufferSummary summary;
    summary.count = samples.size();
    if (samples.empty()) {
        summary.note = "no trace spans observed";
        return summary;
    }
    ...
    summary.note = "bounded diagnostic capture";
    return summary;
}
...
std::string describe_trace_buffer(const TraceBufferSummary& summary) {
    ...
    out << "TraceBuffer[count=" << summary.count
        << ", age=" << summary.average
