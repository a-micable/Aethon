#include "aethon/diagnostics/audit_log.hpp"
...
AuditLog::AuditLog(std::string name) : name_(std::move(name)) {}

void AuditLog::observe(AuditLogSample sample) {
...
void AuditLog::clear_before(std::uint64_t timestamp_ns) {
...
AuditLogSummary AuditLog::summarize() const {
    std::vector<AuditLogSample> copy(samples_.begin(), samples_.end());
    auto summary = summarize_audit_log(copy);
...
std::optional<AuditLogSample> AuditLog::latest(std::string_view label) const {
...
std::vector<AuditLogSample> AuditLog::select(double threshold) const {
    std::vector<AuditLogSample> out;
...
AuditLogSummary summarize_audit_log(const std::vector<AuditLogSample>& samples) {
    ...
    if (samples.empty()) {
        summary.note = "no control events observed";
        return summary;
    }
    ...
    summary.note = "append-only audit entries";
    return summary;
}
...
std::string describe_audit_log(const AuditLogSummary& summary) {
    ...
    out << "AuditLog[count=" << summary.count
